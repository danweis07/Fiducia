...
| Role-based access   | `AdminProtectedRoute` component, RBAC roles in `profiles.role`                               | `apps/web/src/components/admin/AdminProtectedRoute.tsx`                 |
| Route protection    | Lazy-loaded admin routes behind auth guard                                                   | `apps/web/src/routes/adminRoutes.tsx`                                   |
...
| Feature flags      | Per-tenant experiment system (`apps/web/src/pages/admin/Experiments.tsx`)         |
| Approval workflows | JIT permission escalation requires approval for high-value operations             |
| Rate limiting      | Client-side throttle + circuit breaker in `apps/web/src/lib/gateway/throttle.ts`  |
...
| MFA                | TOTP-based MFA configurable per tenant (`apps/web/src/pages/admin/TenantSettings.tsx`) |
| Session management | JWT refresh tokens with configurable expiry                                            |
| SSO                | SAML/OIDC configuration via `apps/web/src/pages/admin/SSOConfiguration.tsx`            |
...
| Control              | Implementation                                                                          |
| -------------------- | --------------------------------------------------------------------------------------- |
| Audit logging        | 49+ action types tracked in `audit_logs` table (`apps/web/src/services/auditLogger.ts`) |
...
| Control             | Implementation                                                            |
| ------------------- | ------------------------------------------------------------------------- |
| Error tracking      | Sentry integration with source maps (`apps/web/src/lib/services/errors/`) |
...
- Onboarding verification workflow in `apps/web/src/pages/admin/TenantOnboarding.tsx`
...
- Compliance dashboard: `apps/web/src/pages/admin/ComplianceCenter.tsx`
