...
| AC-02 | Role-based access control | Admin/Owner/Member roles enforced at route and API level | `src/components/admin/AdminProtectedRoute.tsx` | Complete |
| AC-03 | Admin route protection | Lazy-loaded routes behind authentication guard | `src/routes/adminRoutes.tsx` | Complete |
...
| AC-07 | Least privilege | Users start with minimal permissions; elevation requires approval | `src/hooks/useApprovals.ts` | Complete |

### AU — Audit and Accountability

| ID | Control Objective | Implementation | Evidence Location | Status |
|---|---|---|---|---|
| AU-01 | Action audit logging | 49+ action types tracked with structured metadata | `src/services/auditLogger.ts` | Complete |
...
| AU-03 | Audit log UI | Searchable, filterable admin view with CSV/JSON export | `src/pages/admin/AuditLog.tsx` | Complete |
...
| ID | Control Objective | Implementation | Evidence Location | Status |
|---|---|---|---|---|
| IA-01 | User authentication | Email/password, magic link, SSO (SAML/OIDC) | Supabase Auth + `src/pages/admin/SSOConfiguration.tsx` | Complete |
| IA-02 | Multi-factor authentication | TOTP-based MFA with per-tenant enforcement | `src/pages/admin/TenantSettings.tsx` | Complete |
| IA-03 | Strong Customer Authentication | SCA support for EU PSD2 payment authorization | Payment adapter layer | Complete |
| IA-04 | API authentication | Service account tokens with rate limiting | `src/pages/admin/ApiTokens.tsx` | Complete |
| IA-05 | KYC verification | Identity verification workflow with provider adapters | Adapter pattern in `src/lib/` | Complete |

### IR — Incident Response

| ID | Control Objective | Implementation | Evidence Location | Status |
|---|---|---|---|---|
| IR-01 | Incident detection | Prometheus alerts + Sentry + health checks | `monitoring/`, `src/lib/services/errors/` | Complete |
| IR-02 | Incident management | Full lifecycle tracking (detect → investigate → resolve) | `src/pages/admin/IncidentManager.tsx` | Complete |
...
| ID | Control Objective | Implementation | Evidence Location | Status |
|---|---|---|---|---|
| CM-01 | Change management | Change request workflow with approval gates | `change_requests` table, `src/pages/admin/ChangeTracker.tsx` | Complete |
...
| CM-04 | Feature flags | Per-tenant experiment system | `src/pages/admin/Experiments.tsx` | Complete |
...
| ID | Control Objective | Implementation | Evidence Location | Status |
|---|---|---|---|---|
| SC-01 | Rate limiting | Client-side throttle + circuit breaker | `src/lib/gateway/throttle.ts`, `src/lib/gateway/circuit-breaker.ts` | Complete |
| SC-02 | Input validation | Zod schemas for all form inputs | Various form components | Complete |
| SC-03 | Error handling | ErrorBoundary components + Sentry capture | `src/components/ErrorBoundary.tsx` | Complete |
...
| DP-03 | Data export (DSAR) | Admin data export functionality | `src/pages/admin/DataExport.tsx` | Complete |
