...
import { Sparkles, Zap, Shield, TrendingUp, FileText, ArrowRightLeft, Bell, ArrowRight, BarChart3, Landmark } from "lucide-react";
...
export default function BusinessHubPage() {
  ...
  const pendingInvoices = recentInvoices.filter(i => i.status === "parsed" || i.status === "pending");
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-yellow-800">{t('businessHub.approvalsWaiting')}</p>
                      <p className="text-2xl font-bold text-yellow-900">{approvalSummary?.pendingCount}</p>
                    </div>
                  </div>
                  <Link to="/approvals">
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-blue-800">{t('businessHub.invoicesToReview')}</p>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-green-800">{t('businessHub.weeklyYield')}</p>
                      <p className="text-2xl font-bold text-green-900">{formatCurrency(sweepSummary?.estimatedYieldCents ?? 0)}</p>
                    </div>
                  </div>
                  <Link to="/cash-sweeps">
          ...
          <Card className="group hover:shadow-lg transition-shadow">
            <CardHeader>
              ...
              <CardTitle className="flex items-center gap-2">
                ...
                <Badge variant="default" className="text-xs">{t('businessHub.aiPowered')}</Badge>
              </CardTitle>
              <CardDescription>
                {t('businessHub.zeroTouchAPDesc')}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between text-sm text-muted-foreground mb-4">
                ...
                {pendingInvoices.length > 0 && (
                  <Badge variant="secondary">{t('businessHub.pendingCount', { count: pendingInvoices.length })}</Badge>
                )}
              </div>
              <Link to="/invoices">
          ...
          <Card className="group hover:shadow-lg transition-shadow">
            <CardHeader>
              ...
              <CardTitle className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-xs">{t('businessHub.automated')}</Badge>
              </CardTitle>
              <CardDescription>
                {t('businessHub.smartCashSweepsDesc')}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between text-sm text-muted-foreground mb-4">
                <span>{t('businessHub.activeRulesCount', { count: sweepSummary?.activeRules ?? 0 })}</span>
                <span>{t('businessHub.sweptAmount', { amount: formatCurrency(sweepSummary?.totalSweptCents ?? 0) })}</span>
              </div>
              <Link to="/cash-sweeps">
          ...
          <Card className="group hover:shadow-lg transition-shadow">
            <CardHeader>
              ...
              <CardTitle className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-xs">{t('businessHub.realTime')}</Badge>
              </CardTitle>
              <CardDescription>
                {t('businessHub.jitPermissionsDesc')}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between text-sm text-muted-foreground mb-4">
                <span>{t('businessHub.pendingCount', { count: approvalSummary?.pendingCount ?? 0 })}</span>
                <span>{t('businessHub.avgResponse', { minutes: approvalSummary?.avgResponseMinutes ?? 0 })}</span>
              </div>
              <Link to="/approvals">
                <Button className="w-full" variant="outline">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-3 px-4 font-semibold">{t('businessHub.feature')}</th>
                    <th className="text-left py-3 px-4 font-semibold text-muted-foreground">{t('businessHub.legacy')}</th>
                    <th className="text-left py-3 px-4 font-semibold text-primary">{t('businessHub.fiducia')}</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b">
                    <td className="py-3 px-4 font-medium">{t('businessHub.payingBills')}</td>
                    <td className="py-3 px-4 text-muted-foreground">{t('businessHub.legacyBills')}</td>
                    <td className="py-3 px-4">{t('businessHub.fiduciaBills')}</td>
                  </tr>
                  <tr className="border-b">
                    <td className="py-3 px-4 font-medium">{t('businessHub.permissions')}</td>
                    <td className="py-3 px-4 text-muted-foreground">{t('businessHub.legacyPermissions')}</td>
                  ...
                  <tr className="border-b">
                    ...
                    <td className="py-3 px-4 text-muted-foreground">{t('businessHub.legacyCash')}</td>
                    <td className="py-3 px-4">{t('businessHub.fiduciaCash')}</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 font-medium">{t('businessHub.onboarding')}</td>
                    <td className="py-3 px-4 text-muted-foreground">{t('businessHub.legacyOnboarding')}</td>
