...
const verificationStatusConfig: Record<EKYCVerificationStatus, { label: string; variant: "default" | "secondary" | "destructive" | "outline" }> = {
...
const livenessStatusConfig: Record<LivenessCheckStatus, { label: string; variant: "default" | "secondary" | "destructive" | "outline" }> = {
...
export default function InternationalEKYC() {
  ...
  const { data: providersData, isLoading: providersLoading } = useEKYCProviders(selectedCountry || undefined);
  const { data: verificationsData, isLoading: verificationsLoading, error: verificationsError } = useEKYCVerifications();
  ...
  const handleInitiate = async () => {
    if (!initiateDialog || !initiateForm.documentType.trim() || !initiateForm.documentNumber.trim()) {
    ...
    try {
      ...
      toast({ title: "Verification initiated", description: "Your identity verification has been started." });
  ...
  const handleStartLiveness = async (verificationId: string) => {
    try {
      ...
      toast({ title: "Liveness check started", description: "Follow the on-screen instructions to complete verification." });
    } catch (err) {
    ...
    <div className="max-w-3xl mx-auto py-6 px-4 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Complete international eKYC requirements with document verification and biometric liveness checks.
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {selectedCountry && !providersLoading && providers.length === 0 && (
            <div className="text-sm text-muted-foreground">No providers available for this country.</div>
          )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm">
              ...
              <p className="text-blue-700 mt-0.5">
                Liveness verification requires you to perform specific actions such as blinking, smiling,
                or turning your head. This confirms that a real person is present and prevents spoofing with
                photos or videos.
              </p>
            </div>
          </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="text-xs text-muted-foreground">
                          ...
                          {verification.failedAt && (
                            <span>Failed {new Date(verification.failedAt).toLocaleDateString()}</span>
                          )}
                          {!verification.verifiedAt && !verification.failedAt && verification.createdAt && (
                            <span>Started {new Date(verification.createdAt).toLocaleDateString()}</span>
                          )}
                        </div>
                      </div>

                      {verification.livenessStatus === "not_started" && verification.status === "in_progress" && (
                        <Button
                          size="sm"
                          variant="outline"
      ...
      {/* Initiate Dialog */}
      <Dialog open={!!initiateDialog} onOpenChange={(open) => { if (!open) setInitiateDialog(null); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Start Identity Verification</DialogTitle>
            <DialogDescription>
              {initiateDialog && (
                <>Verify your identity with {initiateDialog.displayName} ({countryOptions.find((c) => c.value === initiateDialog.country)?.label ?? initiateDialog.country}).</>
              )}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setInitiateForm({ ...initiateForm, documentNumber: e.target.value })}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setInitiateDialog(null)}>Cancel</Button>
