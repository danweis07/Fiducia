...
import { useLoanProducts, useCreateLoanApplication, useUploadLoanDocument } from "@/hooks/useLoanOrigination";
...
export default function LoanApplication() {
  ...
  const validateEmployment = useCallback((): boolean => {
    const errs: Record<string, string> = {};
    if (!form.employmentStatus) errs.employmentStatus = t('loanApplication.errors.employmentStatusRequired');
    if (
    ...
    if (!form.annualIncomeDollars.trim()) {
      ...
      if (isNaN(income) || income <= 0) errs.annualIncomeDollars = t('loanApplication.errors.invalidIncome');
    }
    setErrors(errs);
    return Object.keys(errs).length === 0;
  }, [form, t]);
    ...
    if (!form.requestedAmountDollars.trim()) {
      ...
      if (isNaN(amount) || amount <= 0) {
        ...
        if (amount * 100 < selectedProduct.minAmountCents) {
          errs.requestedAmountDollars = t('loanApplication.errors.minimumAmount', { amount: formatCurrency(selectedProduct.minAmountCents) });
        }
        if (amount * 100 > selectedProduct.maxAmountCents) {
          errs.requestedAmountDollars = t('loanApplication.errors.maximumAmount', { amount: formatCurrency(selectedProduct.maxAmountCents) });
        }
      }
    }
  ...
  const termOptions = selectedProduct
    ? Array.from(
        { length: Math.floor((selectedProduct.maxTermMonths - selectedProduct.minTermMonths) / 12) + 1 },
  ...
  return (
    <AppShell>
      <div className="max-w-2xl mx-auto py-8 px-4">
        ...
        {step !== "submitted" && (
          <div className="mb-6">
            <div className="flex items-center justify-between text-sm text-muted-foreground mb-2">
              ...
              <span>
                {t('loanApplication.stepOf', { current: stepIndex + 1, total: STEP_ORDER.length - 1 })}
              </span>
            </div>
        ...
        {step === "submitted" && (
          <SubmittedStep applicationId={applicationId} />
        )}
      </div>
    </AppShell>
  );
