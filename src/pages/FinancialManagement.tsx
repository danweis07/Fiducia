...
export default function FinancialManagement() {
  ...
  const { data: spendingData, isLoading: spendingLoading, error: spendingError } = useQuery<SpendingData>({
    queryKey: ["financial-spending"],
    queryFn: () => gateway.financial.spending(),
  });

  const { data: trendsData, isLoading: trendsLoading, error: trendsError } = useQuery<{ trends: TrendItem[] }>({
    queryKey: ["financial-trends"],
    queryFn: () => gateway.financial.trends({ months: 6 }),
  });

  const { data: budgetsData, isLoading: budgetsLoading, error: budgetsError } = useQuery<BudgetsData>({
    queryKey: ["financial-budgets"],
    queryFn: () => gateway.financial.listBudgets(),
  });

  const { data: recurringData, isLoading: recurringLoading, error: recurringError } = useQuery<RecurringData>({
    queryKey: ["financial-recurring"],
    queryFn: () => gateway.financial.recurring(),
  });

  const { data: netWorthData, isLoading: netWorthLoading, error: netWorthError } = useQuery<NetWorthData>({
    queryKey: ["financial-networth"],
    queryFn: () => gateway.financial.netWorth(),
  });

  const isLoading = spendingLoading || trendsLoading || budgetsLoading || recurringLoading || netWorthLoading;
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 space-y-8">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          {t('financialManagement.subtitle')}
        </p>
      </div>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">{t('financialManagement.totalSpending')}</p>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">{t('financialManagement.totalIncome')}</p>
                <p className="text-2xl font-bold">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">{t('financialManagement.netCashFlow')}</p>
                <p className={cn(
                  "text-2xl font-bold",
      ...
      <Card>
        ...
        <CardContent>
          {categories.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">{t('financialManagement.noSpendingData')}</p>
          ) : (
      ...
      <Card>
        ...
        <CardContent>
          {trends.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">{t('financialManagement.noTrendData')}</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    <th className="text-left py-2 pr-4 font-medium text-muted-foreground">{t('financialManagement.month')}</th>
                    <th className="text-right py-2 px-4 font-medium text-muted-foreground">{t('financialManagement.spending')}</th>
                    <th className="text-right py-2 px-4 font-medium text-muted-foreground">{t('financialManagement.income')}</th>
                    <th className="text-right py-2 pl-4 font-medium text-muted-foreground">{t('financialManagement.savings')}</th>
                  </tr>
                </thead>
                <tbody>
                  {trends.map((trend) => (
                    <tr key={trend.month} className="border-b last:border-0">
                      ...
                      <td className="py-3 px-4 text-right text-red-600">{formatCurrency(trend.spendingCents)}</td>
                      <td className="py-3 px-4 text-right text-green-600">{formatCurrency(trend.incomeCents)}</td>
                      <td className={cn(
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {budgetsData
              ? t('financialManagement.budgetUsed', { spent: formatCurrency(budgetsData.totalSpentCents), total: formatCurrency(budgetsData.totalBudgetCents) })
              : t('financialManagement.budgetsDesc')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {budgets.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">{t('financialManagement.noBudgets')}</p>
          ) : (
                ...
                return (
                  <div key={b.budgetId} className="rounded-lg border p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      {b.isOverBudget && (
                        <Badge variant="destructive" className="text-xs">{t('financialManagement.overBudget')}</Badge>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between text-xs text-muted-foreground">
                      <span>{t('financialManagement.spent', { amount: formatCurrency(b.spentCents) })}</span>
                      <span>{t('financialManagement.limit', { amount: formatCurrency(b.limitCents) })}</span>
                    </div>
                  </div>
                );
              })}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {recurringData
              ? t('financialManagement.recurringCost', { monthly: formatCurrency(recurringData.totalMonthlyCents), annual: formatCurrency(recurringData.totalAnnualCents) })
              : t('financialManagement.recurringDesc')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {subscriptions.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">{t('financialManagement.noRecurring')}</p>
          ) : (
                ...
                <div key={sub.id} className="flex items-center gap-4 py-3 first:pt-0 last:pb-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {!sub.isActive && (
                        <Badge variant="outline" className="text-xs">{t('financialManagement.inactive')}</Badge>
                      )}
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {sub.frequency} &middot; {t('financialManagement.next')}: {new Date(sub.nextExpectedDate).toLocaleDateString(undefined, {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                      })}
      ...
      <Card>
        <CardHeader>
          ...
          {netWorthData && (
            <CardDescription>
              {t('financialManagement.asOfDate', { date: new Date(netWorthData.date).toLocaleDateString(undefined, {
                month: "long",
                day: "numeric",
                year: "numeric",
              }) })}
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-lg border p-4 text-center">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">{t('financialManagement.totalAssets')}</p>
              <p className="text-xl font-bold text-green-600 mt-1">
                {formatCurrency(netWorthData?.totalAssetsCents ?? 0)}
              </p>
            </div>
            <div className="rounded-lg border p-4 text-center">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">{t('financialManagement.totalLiabilities')}</p>
              <p className="text-xl font-bold text-red-600 mt-1">
                {formatCurrency(netWorthData?.totalLiabilitiesCents ?? 0)}
              </p>
            </div>
            <div className="rounded-lg border p-4 text-center">
              <p className="text-xs text-muted-foreground uppercase tracking-wide">{t('financialManagement.netWorth')}</p>
              <p className={cn(
