...
import { Target, Plus, ArrowUpRight, ArrowDownLeft, Trash2, TrendingUp, CheckCircle2, AlertCircle } from "lucide-react";
...
import {
  useSavingsGoals, useCreateSavingsGoal, useDeleteSavingsGoal,
  useContributeToGoal, useWithdrawFromGoal, useGoalSummary,
...
export default function SavingsGoals() {
  ...
  const resetCreateForm = () => {
    setGoalName(''); setGoalTarget(''); setGoalDate(''); setGoalAccount('');
    setGoalEmoji(''); setAutoContribute(false); setAutoAmount(''); setAutoFreq('monthly');
  };

  const handleCreate = async () => {
    const targetCents = Math.round(parseFloat(goalTarget) * 100);
    if (!goalName || !targetCents || targetCents <= 0 || !goalAccount) {
      toast({ title: t('savingsGoals.fillRequiredFields'), variant: "destructive" }); return;
    }
    try {
      await createGoal.mutateAsync({
        ...
        autoContribute,
        autoContributeAmountCents: autoContribute && autoAmount ? Math.round(parseFloat(autoAmount) * 100) : undefined,
  ...
  const handleContribute = async () => {
    ...
    if (!cents || cents <= 0) { toast({ title: t('savingsGoals.invalidAmount'), variant: "destructive" }); return; }
    try {
      await contribute.mutateAsync({ goalId: contributeGoal.id, amountCents: cents });
      toast({ title: t('savingsGoals.contributionAdded'), description: t('savingsGoals.contributionAddedDesc', { amount: formatCurrency(cents), name: contributeGoal.name }) });
      setAmount(''); setContributeGoal(null);
    } catch (e) { handleError(e); }
  };
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      toast({ title: t('savingsGoals.withdrawalCompleted'), description: t('savingsGoals.withdrawalCompletedDesc', { amount: formatCurrency(cents), name: withdrawGoal.name }) });
      setAmount(''); setWithdrawGoal(null);
    } catch (e) { handleError(e); }
  };
  ...
  const handleDelete = async (goal: SavingsGoal) => {
    try {
      ...
      toast({ title: t('savingsGoals.goalDeleted'), description: t('savingsGoals.goalDeletedDesc', { name: goal.name }) });
    } catch (e) { handleError(e); }
  };
  ...
  return (
    <AppShell>
      <main id="main-content" className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            <DialogTrigger asChild>
              <Button><Plus className="h-4 w-4 mr-2" />{t('savingsGoals.newGoal')}</Button>
            </DialogTrigger>
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
                <div className="grid grid-cols-[1fr_60px] gap-2">
                  <div className="space-y-2">
                    ...
                    <Input value={goalName} onChange={(e) => setGoalName(e.target.value)} placeholder={t('savingsGoals.goalNamePlaceholder')} />
                  </div>
                  <div className="space-y-2">
                    <Label>{t('savingsGoals.icon')}</Label>
                    <Input value={goalEmoji} onChange={(e) => setGoalEmoji(e.target.value)} placeholder="🏖" maxLength={2} className="text-center text-lg" />
                  </div>
                </div>
                <div className="space-y-2">
                  <Label>{t('savingsGoals.targetAmount')}</Label>
                  <Input type="number" min="1" step="0.01" value={goalTarget}
                    onChange={(e) => setGoalTarget(e.target.value)} placeholder="5000.00" />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={goalAccount} onValueChange={setGoalAccount}>
                    <SelectTrigger><SelectValue placeholder={t('savingsGoals.selectAccount')} /></SelectTrigger>
                    <SelectContent>
                ...
                {autoContribute && (
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-2">
                      ...
                      <Input type="number" min="1" step="0.01" value={autoAmount}
                        onChange={(e) => setAutoAmount(e.target.value)} placeholder="100.00" />
                    </div>
                    <div className="space-y-2">
                      <Label>{t('savingsGoals.frequency')}</Label>
                      <Select value={autoFreq} onValueChange={(v) => setAutoFreq(v as 'weekly' | 'biweekly' | 'monthly')}>
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setCreateOpen(false)}>{t('savingsGoals.cancel')}</Button>
                <Button onClick={handleCreate} disabled={createGoal.isPending}>
                  {createGoal.isPending ? t('savingsGoals.creating') : t('savingsGoals.createGoalBtn')}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        ...
        {/* Goals Grid */}
        {goals.length === 0 ? (
          <EmptyState icon={<Target className="h-10 w-10" />} title={t('savingsGoals.noGoals')}
                ...
                <CardContent className="space-y-3">
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="text-muted-foreground">{formatCurrency(goal.targetAmountCents)}</span>
                    </div>
                    <Progress value={goal.progressPercent} className="h-2" />
                    <div className="flex justify-between mt-1">
                      <span className="text-xs text-muted-foreground">{t('savingsGoals.percentComplete', { percent: goal.progressPercent })}</span>
                      {goal.targetDate && (
                  ...
                  {goal.status === 'active' && goal.targetDate && (
                    <div className="flex items-center gap-1 text-xs">
                      {goal.isOnTrack
                        ? <><TrendingUp className="h-3 w-3 text-green-500" /><span className="text-green-600">{t('savingsGoals.onTrack')}</span></>
                        : <><AlertCircle className="h-3 w-3 text-amber-500" /><span className="text-amber-600">{t('savingsGoals.behindSchedule')}</span></>
                      }
                    </div>
                  )}
                  {goal.autoContribute && goal.autoContributeAmountCents && (
                    <p className="text-xs text-muted-foreground">
                      Auto: {formatCurrency(goal.autoContributeAmountCents)} {goal.autoContributeFrequency}
                    </p>
                  )}
                  <div className="flex gap-2">
                    {goal.status === 'active' && (
                      <>
                        <Button size="sm" className="flex-1" onClick={() => { setContributeGoal(goal); setAmount(''); }}>
                          <ArrowUpRight className="h-3 w-3 mr-1" />{t('savingsGoals.add')}
                        </Button>
                        <Button size="sm" variant="outline" className="flex-1"
                          onClick={() => { setWithdrawGoal(goal); setAmount(''); }}
                          disabled={goal.currentAmountCents === 0}>
                          <ArrowDownLeft className="h-3 w-3 mr-1" />{t('savingsGoals.withdraw')}
                        </Button>
                      </>
                    )}
                    <Button size="sm" variant="ghost" className="text-destructive"
                      onClick={() => handleDelete(goal)} disabled={deleteGoal.isPending}>
        ...
        <Dialog open={!!contributeGoal} onOpenChange={(o) => !o && setContributeGoal(null)}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {t('savingsGoals.currentOfTarget', { current: contributeGoal ? formatCurrency(contributeGoal.currentAmountCents) : '', target: contributeGoal ? formatCurrency(contributeGoal.targetAmountCents) : '' })}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-2">
              <Label>{t('savingsGoals.amountDollar')}</Label>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setContributeGoal(null)}>{t('savingsGoals.cancel')}</Button>
        ...
        <Dialog open={!!withdrawGoal} onOpenChange={(o) => !o && setWithdrawGoal(null)}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t('savingsGoals.withdrawFrom', { name: withdrawGoal?.name })}</DialogTitle>
              <DialogDescription>
                {t('savingsGoals.available', { amount: withdrawGoal ? formatCurrency(withdrawGoal.currentAmountCents) : '' })}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-2">
              <Label>{t('savingsGoals.amountDollar')}</Label>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setWithdrawGoal(null)}>{t('savingsGoals.cancel')}</Button>
