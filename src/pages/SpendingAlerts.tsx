...
import { useSpendingAlerts, useCreateAlert, useUpdateAlert, useDeleteAlert, useAlertHistory, useAlertSummary } from "@/hooks/useSpendingAlerts";
...
export default function SpendingAlertsPage() {
  ...
  const handleSave = () => {
    ...
    if (!formName) { toast({ title: t('spendingAlerts.nameRequired'), variant: "destructive" }); return; }
    ...
    if (editingAlert) {
      updateAlert.mutate({
        ...
        onSuccess: () => { toast({ title: t('spendingAlerts.alertUpdated') }); setDialogOpen(false); resetForm(); },
        onError: () => toast({ title: t('spendingAlerts.failedToUpdateAlert'), variant: "destructive" }),
      });
    } else {
      createAlert.mutate({
        name: formName,
        ...
        onSuccess: () => { toast({ title: t('spendingAlerts.alertCreated') }); setDialogOpen(false); resetForm(); },
        onError: () => toast({ title: t('spendingAlerts.failedToCreateAlert'), variant: "destructive" }),
      });
    }
  };
  ...
  const needsThreshold = (alertType: SpendingAlertType) => alertType !== "international_transaction";
      ...
      <main id="main-content" className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogTrigger asChild>
              <Button onClick={openCreate}><Plus className="w-4 h-4 mr-2" />{t('spendingAlerts.newAlert')}</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editingAlert ? t('spendingAlerts.editAlert') : t('spendingAlerts.createAlert')}</DialogTitle>
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input value={formName} onChange={e => setFormName(e.target.value)} placeholder={t('spendingAlerts.alertNamePlaceholder')} />
                </div>
                <div className="space-y-2">
                  <Label>{t('spendingAlerts.alertType')}</Label>
                  <Select value={formType} onValueChange={v => setFormType(v as SpendingAlertType)}>
                ...
                {needsThreshold(formType) && (
                  <div className="space-y-2">
                    ...
                    <Input type="number" min="0" step="0.01" value={formThreshold} onChange={e => setFormThreshold(e.target.value)} placeholder="100.00" />
                  </div>
                )}
                <div className="space-y-2">
                  <Label>{t('spendingAlerts.accountOptional')}</Label>
                  <Select value={formAccountId} onValueChange={setFormAccountId}>
                    <SelectTrigger><SelectValue placeholder={t('spendingAlerts.allAccounts')} /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">{t('spendingAlerts.allAccounts')}</SelectItem>
                      {accounts.filter(a => a.status === "active").map(a => (
                        <SelectItem key={a.id} value={a.id}>{a.nickname || a.type} - {a.accountNumberMasked}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                  ...
                  <div className="flex gap-4">
                    {CHANNELS.map(ch => (
                      <div key={ch} className="flex items-center gap-2">
                        <Checkbox id={`ch-${ch}`} checked={formChannels.has(ch)} onCheckedChange={() => toggleChannel(ch)} />
                        <Label htmlFor={`ch-${ch}`} className="capitalize">{ch}</Label>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setDialogOpen(false)}>{t('spendingAlerts.cancel')}</Button>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t('spendingAlerts.activeRules')}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t('spendingAlerts.triggeredThisWeek')}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="pt-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t('spendingAlerts.triggeredThisMonth')}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <Tabs defaultValue="rules">
          ...
          <TabsContent value="rules" className="space-y-4">
            {isLoading ? (
              <Card><CardContent className="py-8 text-center text-muted-foreground">{t('spendingAlerts.loading')}</CardContent></Card>
            ) : alerts.length === 0 ? (
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <Button className="mt-4" onClick={openCreate}><Plus className="w-4 h-4 mr-2" />{t('spendingAlerts.createFirstAlert')}</Button>
                </CardContent>
              </Card>
            ) : (
              <Card>
                  ...
                  <TableBody>
                    {alerts.map((alert: SpendingAlertRule) => (
                      <TableRow key={alert.id}>
                        ...
                        <TableCell>
                          <Badge variant="outline">{ALERT_TYPES.find(at => at.value === alert.alertType)?.label ?? alert.alertType}</Badge>
                        </TableCell>
                        <TableCell>{formatCents(alert.thresholdCents)}</TableCell>
                        <TableCell>
                          <div className="flex gap-1">{alert.channels.map(ch => <Badge key={ch} variant="secondary" className="text-xs capitalize">{ch}</Badge>)}</div>
                        </TableCell>
                        <TableCell>
                          <Switch checked={alert.isEnabled} onCheckedChange={v => handleToggle(alert, v)} />
                        </TableCell>
                        <TableCell>
              ...
              <CardContent>
                {historyEvents.length === 0 ? (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {historyEvents.map((event: { id: string; triggeredAt: string; ruleName: string; alertType: string; message: string; amountCents: number | null }) => (
                        <TableRow key={event.id}>
                          <TableCell>{new Date(event.triggeredAt).toLocaleDateString()}</TableCell>
                          <TableCell className="font-medium">{event.ruleName}</TableCell>
                          <TableCell><Badge variant="outline">{event.alertType.replace(/_/g, " ")}</Badge></TableCell>
