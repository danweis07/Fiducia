...
const statusConfig: Record<string, { labelKey: string; className: string; icon: React.ElementType }> = {
  pending: { labelKey: "checkOrdering.statusPending", className: "bg-yellow-100 text-yellow-700", icon: Clock },
  processing: { labelKey: "checkOrdering.statusProcessing", className: "bg-blue-100 text-blue-700", icon: Package },
  shipped: { labelKey: "checkOrdering.statusShipped", className: "bg-indigo-100 text-indigo-700", icon: Truck },
  delivered: { labelKey: "checkOrdering.statusDelivered", className: "bg-green-100 text-green-700", icon: CheckCircle },
  cancelled: { labelKey: "checkOrdering.statusCancelled", className: "bg-gray-100 text-gray-500", icon: XCircle },
};
...
export default function CheckOrdering() {
  ...
  function handleOrder() {
    if (!selectedStyle || !selectedAccountId) {
      toast({ title: t('checkOrdering.missingSelection'), description: t('checkOrdering.missingSelectionDesc'), variant: "destructive" });
      return;
    }
    createOrder.mutate(
      {
      ...
      {
        onSuccess: () => {
          toast({ title: t('checkOrdering.orderPlaced'), description: t('checkOrdering.orderPlacedDesc') });
          setActiveTab("history");
          setSelectedStyle(null);
        },
        onError: () => {
          toast({ title: t('checkOrdering.orderFailed'), description: t('checkOrdering.orderFailedDesc'), variant: "destructive" });
        },
      }
    );
  }
  ...
  function handleCancel(orderId: string) {
    cancelOrder.mutate(orderId, {
      onSuccess: () => toast({ title: t('checkOrdering.orderCancelled'), description: t('checkOrdering.orderCancelledDesc') }),
      onError: () => toast({ title: t('checkOrdering.cancelFailed'), description: t('checkOrdering.cancelFailedDesc'), variant: "destructive" }),
    });
  }

  if (stylesLoading) return <AppShell><PageSkeleton /></AppShell>;
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 space-y-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList>
            ...
            <TabsTrigger value="history">{t('checkOrdering.orderHistory')} ({orders.length})</TabsTrigger>
          </TabsList>
                ...
                >
                  ...
                  <CardContent className="pt-3 pb-4">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <Badge variant="outline" className="text-xs capitalize">{style.category}</Badge>
                    </div>
                    <p className="text-xs text-muted-foreground mb-2 line-clamp-2">{style.description}</p>
                    <div className="text-sm font-medium text-primary">
                      {t('checkOrdering.pricePerBox', { price: formatCurrency(style.pricePerBoxCents) })}
                    </div>
                  </CardContent>
                </Card>
              ))}
            ...
            {selectedStyle && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{t('checkOrdering.selected', { name: selectedStyle.name })}</CardDescription>
                </CardHeader>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select value={selectedAccountId} onValueChange={setSelectedAccountId}>
                        <SelectTrigger><SelectValue placeholder={t('checkOrdering.selectCheckingAccount')} /></SelectTrigger>
                        <SelectContent>
                        ...
                        <SelectContent>
                          {(configData?.quantities ?? [50, 100, 150, 200]).map((q) => (
                            <SelectItem key={q} value={String(q)}>
                              {t('checkOrdering.checksCount', { count: q, boxes: q / 50, boxLabel: q / 50 === 1 ? t('checkOrdering.box') : t('checkOrdering.boxes') })}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                        ...
                        <SelectContent>
                          {shippingOptions.map((opt) => (
                            <SelectItem key={opt.method} value={opt.method}>
                              {opt.label} {opt.costCents > 0 ? `(${formatCurrency(opt.costCents)})` : `(${t('checkOrdering.free')})`}
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                  ...
                  <div className="border-t pt-4 mt-4">
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between">
                        <span>{selectedStyle.name} x {boxCount} {boxCount === 1 ? t('checkOrdering.box') : t('checkOrdering.boxes')}</span>
                        <span>{formatCurrency(checksCost)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>{t('checkOrdering.shipping')} ({shippingMethod})</span>
                        <span>{shippingCost > 0 ? formatCurrency(shippingCost) : t('checkOrdering.free')}</span>
                      </div>
                    ...
                    >
                      ...
                      {createOrder.isPending ? t('checkOrdering.placingOrder') : t('checkOrdering.placeOrder')}
                    </Button>
                  </div>
                </CardContent>
              </Card>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          ...
                          <div className="text-sm text-muted-foreground">
                            {t('checkOrdering.orderDetails', { quantity: order.quantity, account: order.accountMasked, method: order.shippingMethod })}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            {t('checkOrdering.ordered', { date: new Date(order.createdAt).toLocaleDateString() })}
                            {order.trackingNumber && ` \u00B7 ${t('checkOrdering.tracking', { number: order.trackingNumber })}`}
                            {order.estimatedDeliveryDate && ` \u00B7 ${t('checkOrdering.estDelivery', { date: new Date(order.estimatedDeliveryDate).toLocaleDateString() })}`}
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="font-semibold">{formatCurrency(order.totalCostCents)}</div>
