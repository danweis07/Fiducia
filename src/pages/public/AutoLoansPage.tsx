...
const benefits = [
  { icon: Zap, title: 'Quick 24-Hour Approval', desc: 'Get a decision within 24 hours of applying — no waiting around.' },
  { icon: Clock, title: 'Flexible Terms 36-84 Months', desc: 'Choose the repayment schedule that fits your budget.' },
  { icon: DollarSign, title: 'No Prepayment Penalty', desc: 'Pay off your loan early without any extra fees.' },
  { icon: Car, title: 'Dealership or Private Party', desc: 'Finance from any dealership or a private party seller.' },
  { icon: ShieldCheck, title: 'GAP Insurance Available', desc: 'Protect yourself from owing more than your car is worth.' },
];
...
const comparison = [
  ...
  { feature: 'Pre-approval', creditUnion: 'Yes, shop with confidence', dealership: 'Rarely offered' },
  ...
  return (
    <PublicShell tenantName="Demo CU">
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-slate-600 text-center mb-12 max-w-2xl mx-auto">
            Whether you&apos;re buying new, used, or looking to refinance, we have the right loan for you.
          </p>
            ...
            {products.map((product) => (
              ...
              >
                <CardHeader className="text-center">
                  <div className={`w-14 h-14 ${product.color === 'green' ? 'bg-green-100' : product.color === 'blue' ? 'bg-blue-100' : 'bg-slate-100'} rounded-full flex items-center justify-center mx-auto mb-3`}>
                    <product.icon className={`h-7 w-7 ${product.color === 'green' ? 'text-green-600' : product.color === 'blue' ? 'text-blue-600' : 'text-slate-600'}`} />
                  </div>
                  <CardTitle className="text-xl text-slate-900">{product.title}</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className={`${product.color === 'green' ? 'bg-green-50' : 'bg-blue-50'} rounded-lg p-4 mb-4 text-center`}>
                    <p className="text-sm text-slate-600">Rates as low as</p>
                    <p className={`text-4xl font-bold ${product.color === 'green' ? 'text-green-600' : 'text-blue-600'}`}>{product.rate}%</p>
      ...
      <section className="py-16 px-4 bg-slate-50">
        <div className="max-w-5xl mx-auto">
          <h2 className="text-3xl font-bold text-slate-900 text-center mb-12">Benefits of a Demo CU Auto Loan</h2>
      ...
      <section className="py-16 px-4 bg-white">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-bold text-slate-900 text-center mb-4">Why Finance with Demo CU?</h2>
          <p className="text-slate-600 text-center mb-12 max-w-2xl mx-auto">
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-slate-100">
                  ...
                  <th className="text-left p-4 text-slate-500 font-semibold">Dealership Financing</th>
                </tr>
              </thead>
              <tbody>
                {comparison.map((row) => (
      ...
      <section className="py-16 px-4 bg-blue-600 text-white">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-blue-100 mb-8 text-lg">
            Apply online in minutes and get pre-approved before you shop. Your dream car is closer than you think.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <p className="text-blue-200 text-xs mt-6">
            *APR = Annual Percentage Rate. Rates are subject to change and based on creditworthiness,
            loan amount, term, and vehicle age. All loans subject to credit approval.
          </p>
        </div>
      </section>
    </PublicShell>
