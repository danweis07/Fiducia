...
export default function SSOConfiguration() {
  ...
  const handleSave = useCallback(async (config: SSOProviderConfig, type: string) => {
    setSaving(type);
    try {
      const result = await saveMutation.mutateAsync(config);
      if (type === 'saml' && result) setSaml((prev) => ({ ...prev, id: (result as { provider?: { id: string } })?.provider?.id }));
      if (type === 'oidc' && result) setOidc((prev) => ({ ...prev, id: (result as { provider?: { id: string } })?.provider?.id }));
      toast({ title: t('sso.toasts.configSaved'), description: t('sso.toasts.configSavedDesc', { type: type.toUpperCase() }) });
    } catch {
      toast({ title: t('sso.toasts.saveFailed'), description: t('sso.toasts.saveFailedDesc'), variant: 'destructive' });
    } finally {
      setSaving(null);
    }
  }, [saveMutation]);
  ...
  const handleTest = useCallback(async (config: SSOProviderConfig, type: string) => {
    ...
    try {
      ...
      toast({ title: t('sso.toasts.connectionSuccessful'), description: t('sso.toasts.connectionSuccessfulDesc', { type: type.toUpperCase() }) });
    } catch {
      toast({ title: t('sso.toasts.connectionFailed'), description: t('sso.toasts.connectionFailedDesc', { type: type.toUpperCase() }), variant: 'destructive' });
    } finally {
      setTesting(null);
    }
  }, []);
  ...
  const renderSharedSettings = (config: SSOProviderConfig, setConfig: React.Dispatch<React.SetStateAction<SSOProviderConfig>>) => (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={config.defaultRole} onValueChange={(v) => setConfig((p) => ({ ...p, defaultRole: v }))}>
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2">
          {saml.isEnabled && <Badge variant="default" className="gap-1"><Shield className="h-3 w-3" /> {t('sso.samlActive')}</Badge>}
          {oidc.isEnabled && <Badge variant="default" className="gap-1"><Key className="h-3 w-3" /> {t('sso.oidcActive')}</Badge>}
          {!saml.isEnabled && !oidc.isEnabled && (
            <Badge variant="secondary" className="gap-1"><AlertTriangle className="h-3 w-3" /> {t('sso.noSsoConfigured')}</Badge>
          )}
        </div>
      </div>
        ...
        <TabsContent value="saml" className="mt-4 space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>{t('sso.saml.sloUrl')} <span className="text-xs text-slate-400">({t('sso.optional')})</span></Label>
                <Input
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              className="gap-1.5"
            >
              {testing === 'saml' ? <Loader2 className="h-4 w-4 animate-spin" /> : <ExternalLink className="h-4 w-4" />}
        ...
        <TabsContent value="oidc" className="mt-4 space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <Button
              ...
              className="gap-1.5"
            >
              {testing === 'oidc' ? <Loader2 className="h-4 w-4 animate-spin" /> : <ExternalLink className="h-4 w-4" />}
              {t('sso.testConnection')}
            </Button>
          </div>
        </TabsContent>
