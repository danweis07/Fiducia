...
function actionCategory(action: AdminAuditAction): { label: string; color: string } {
  ...
  if (action.includes("update") || action.includes("configure") || action.includes("toggle") || action.includes("reset")) {
...
export default function AuditLog() {
  ...
  const exportData = useCallback((format: 'csv' | 'json') => {
    ...
    if (format === 'csv') {
      ...
      const rows = filtered.map(e => [e.timestamp, e.user, e.action, e.entityType, e.ipAddress, `"${e.details}"`].join(','));
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">{t('auditLog.title')}</h1>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => exportData('csv')}>{t('auditLog.exportCsv')}</DropdownMenuItem>
            <DropdownMenuItem onClick={() => exportData('json')}>{t('auditLog.exportJson')}</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input placeholder={t('auditLog.searchPlaceholder')} value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
            </div>
                ...
                {filtered.map((entry) => {
                  ...
                  return (
                    <TableRow key={entry.id}>
                      <TableCell className="text-sm text-slate-500 whitespace-nowrap">{formatTimestamp(entry.timestamp)}</TableCell>
                      ...
                      <TableCell className="text-sm text-slate-500 font-mono">{entry.ipAddress}</TableCell>
                      <TableCell className="text-sm text-slate-500 max-w-[300px] truncate">{entry.details}</TableCell>
                    </TableRow>
                  );
                })}
                {filtered.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-8 text-slate-400">{t('auditLog.noEntriesFound')}</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
