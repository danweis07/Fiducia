...
describe('ComplianceDashboard (ComplianceCenter)', () => {
  ...
  it('renders the compliance score section', async () => {
    render(createElement(ComplianceCenter), { wrapper: createWrapper() });
    await waitFor(() => {
  ...
  it('shows empty KYC table when no reviews pending', async () => {
    render(createElement(ComplianceCenter), { wrapper: createWrapper() });
    await waitFor(() => {
      expect(screen.getByText('Pending KYC Verifications')).toBeTruthy();
      expect(screen.getByText('No KYC reviews pending.')).toBeTruthy();
    });
  });
});
