        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {t('cdpManager.config.connectionDescription')}
              </CardDescription>
            </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {(config?.consentCategories ?? ["functional", "analytics", "marketing"]).map((cat) => (
...
function DestinationsTab() {
  ...
  const handleCreate = () => {
    ...
    createDest.mutate(
      ...
      },
      {
        ...
        onError: () => {
          toast({ title: t('cdpManager.destinations.toasts.createFailed'), variant: "destructive" });
        },
      }
    );
  };
  ...
  const handleToggle = (id: string, currentEnabled: boolean) => {
    updateDest.mutate(
      ...
      {
        onSuccess: () => {
          toast({ title: currentEnabled ? t('cdpManager.destinations.toasts.disabled') : t('cdpManager.destinations.toasts.enabled') });
        },
      }
    );
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {t('cdpManager.destinations.description')}
        </p>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={newType} onValueChange={setNewType}>
                  <SelectTrigger>
                    <SelectValue placeholder={t('cdpManager.destinations.dialog.typePlaceholder')} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setDialogOpen(false)}>{t('cdpManager.destinations.dialog.cancel')}</Button>
              <Button onClick={handleCreate} disabled={!newName || !newType || createDest.isPending}>
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <Power className={`h-4 w-4 ${dest.enabled ? "text-green-600" : "text-muted-foreground"}`} />
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
              ...
              <CardContent>
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className={dest.enabled ? "text-green-700" : "text-muted-foreground"}>
                    {dest.enabled ? t('cdpManager.destinations.active') : t('cdpManager.destinations.disabled')}
                  </span>
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  {t('cdpManager.destinations.type')}: {DESTINATION_TYPES.find((dt) => dt.value === dest.type)?.label ?? dest.type}
                </p>
                {dest.lastSyncAt && (
                  <p className="text-xs text-muted-foreground mt-1">
                    {t('cdpManager.destinations.lastSync')}: {new Date(dest.lastSyncAt).toLocaleDateString()}
                  </p>
                )}
              </CardContent>
            </Card>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            <CardContent className="pt-6">
              ...
              <p className="text-xs text-muted-foreground">{t('cdpManager.events.totalEvents7d')}</p>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="pt-6">
