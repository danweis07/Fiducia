...
const switchStatusConfig: Record<string, { labelKey: string; className: string; icon: React.ElementType }> = {
  pending: { labelKey: "directDeposit.statusPending", className: "bg-yellow-100 text-yellow-700", icon: Clock },
  awaiting_login: { labelKey: "directDeposit.statusAwaitingLogin", className: "bg-blue-100 text-blue-700", icon: Loader2 },
  processing: { labelKey: "directDeposit.statusProcessing", className: "bg-indigo-100 text-indigo-700", icon: Loader2 },
  completed: { labelKey: "directDeposit.statusCompleted", className: "bg-green-100 text-green-700", icon: CheckCircle },
  failed: { labelKey: "directDeposit.statusFailed", className: "bg-red-100 text-red-700", icon: AlertCircle },
  cancelled: { labelKey: "directDeposit.statusCancelled", className: "bg-gray-100 text-gray-500", icon: XCircle },
};
...
export default function DirectDeposit() {
  ...
  function handleInitiate() {
    if (!selectedEmployerId || !selectedAccountId) {
      toast({ title: t('directDeposit.missingSelection'), description: t('directDeposit.missingSelectionDesc'), variant: "destructive" });
      return;
    }
    ...
    if (allocationType === "fixed_amount") {
      ...
      if (!cents || cents <= 0) {
        toast({ title: t('directDeposit.invalidAmount'), description: t('directDeposit.invalidAmountDesc'), variant: "destructive" });
        return;
      }
    ...
    if (allocationType === "partial") {
      ...
      if (!pct || pct <= 0 || pct > 100) {
        toast({ title: t('directDeposit.invalidPercentage'), description: t('directDeposit.invalidPercentageDesc'), variant: "destructive" });
        return;
      }
    ...
    initiate.mutate(params, {
      onSuccess: (data) => {
        toast({ title: t('directDeposit.switchInitiated'), description: t('directDeposit.switchInitiatedDesc') });
        if (data.widgetUrl) {
      ...
      onError: () => {
        toast({ title: t('directDeposit.switchFailed'), description: t('directDeposit.switchFailedDesc'), variant: "destructive" });
      },
    });
  }
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 space-y-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList>
            ...
            <TabsTrigger value="history">{t('directDeposit.switchHistory', { count: switches.length })}</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="switch" className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {employersLoading ? (
                  <div className="text-center py-4 text-muted-foreground">{t('directDeposit.searching')}</div>
                    ...
                    {employers.map((emp) => (
                      ...
                      >
                        <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-muted flex items-center justify-center overflow-hidden">
                          {emp.logoUrl ? (
                            <img src={emp.logoUrl} alt={emp.name} className="w-8 h-8 object-contain" />
                          ) : (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select value={selectedAccountId} onValueChange={setSelectedAccountId}>
                        <SelectTrigger><SelectValue placeholder={t('directDeposit.selectAccount')} /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={allocationType} onValueChange={(v) => setAllocationType(v as AllocationTypeValue)}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          <SelectItem value="full">{t('directDeposit.fullPaycheck')}</SelectItem>
                          <SelectItem value="partial">{t('directDeposit.percentageOfPaycheck')}</SelectItem>
                          <SelectItem value="fixed_amount">{t('directDeposit.fixedDollarAmount')}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  ...
                  >
                    {initiate.isPending ? (
                      <>
                        <Loader2 className="h-4 w-4 mr-2 animate-spin" /> {t('directDeposit.initiating')}
                      </>
                    ) : (
                      <>
          ...
          <TabsContent value="history" className="space-y-4">
            {switchesLoading ? (
              ...
              switches.map((sw) => {
                ...
                const cancellable = sw.status === ("pending" as DirectDepositSwitchStatus) || sw.status === ("awaiting_login" as DirectDepositSwitchStatus);
                return (
                  <Card key={sw.id}>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="space-y-1">
                          ...
                          <div className="text-sm text-muted-foreground">
                            Account {sw.accountMasked} &middot; {allocationLabels[sw.allocationType] ?? sw.allocationType}
                            {sw.allocationAmountCents != null && ` (${formatCurrency(sw.allocationAmountCents)})`}
                            {sw.allocationPercentage != null && ` (${sw.allocationPercentage}%)`}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            Started {new Date(sw.createdAt).toLocaleDateString()}
                            {sw.completedAt && ` \u00B7 Completed ${new Date(sw.completedAt).toLocaleDateString()}`}
