...
import { useStopPayments, useCreateStopPayment, useCancelStopPayment, useRenewStopPayment, useStopPaymentFee } from "@/hooks/useStopPayments";
...
const STATUS_STYLES: Record<StopPaymentStatus, { variant: "default" | "secondary" | "destructive" | "outline"; labelKey: string }> = {
...
export default function StopPayments() {
  ...
  const [renewDuration, setRenewDuration] = useState<"6months" | "12months" | "permanent">("6months");
  ...
  const handleRenew = async () => {
    ...
    try {
      ...
      toast({ title: t('stopPayments.renewed'), description: t('stopPayments.renewedDesc', { duration: t(DURATION_LABEL_KEYS[renewDuration]) }) });
    } catch (err) {
  ...
  if (accountsLoading || spLoading) return <AppShell><PageSkeleton /></AppShell>;
    ...
    <AppShell>
        ...
        <div className="flex items-center justify-between">
          ...
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            <DialogTrigger asChild>
              <Button><Plus className="w-4 h-4 mr-2" />{t('stopPayments.newStopPayment')}</Button>
            </DialogTrigger>
            <DialogContent className="max-w-md">
              ...
              <div className="grid gap-4 py-2">
                <div>
                  ...
                  <Select value={accountId} onValueChange={setAccountId}>
                    <SelectTrigger><SelectValue placeholder={t('stopPayments.selectAccount')} /></SelectTrigger>
                    <SelectContent>
                      {accounts.map((a) => (
                ...
                <div>
                  ...
                  <Input value={checkNumber} onChange={(e) => setCheckNumber(e.target.value)} placeholder={t('stopPayments.checkNumberPlaceholder')} />
                </div>
                ...
                {isRange && (
                  <div>
                    ...
                    <Input value={checkNumberEnd} onChange={(e) => setCheckNumberEnd(e.target.value)} placeholder={t('stopPayments.endCheckNumberPlaceholder')} />
                  </div>
                )}
                <div>
                  <Label>{t('stopPayments.payeeNameOptional')}</Label>
                  <Input value={payeeName} onChange={(e) => setPayeeName(e.target.value)} placeholder={t('stopPayments.payeeNamePlaceholder')} />
                </div>
                <div>
                  <Label>{t('stopPayments.amountOptional')}</Label>
                  <Input type="number" min="0" step="0.01" value={amountStr} onChange={(e) => setAmountStr(e.target.value)} placeholder={t('stopPayments.amountPlaceholder')} />
                </div>
                <div>
                  <Label>{t('stopPayments.reason')}</Label>
                  <Textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder={t('stopPayments.reasonPlaceholder')} rows={2} />
                </div>
                <div>
                  <Label>{t('stopPayments.duration')}</Label>
                  <Select value={duration} onValueChange={(v) => setDuration(v as "6months" | "12months" | "permanent")}>
              ...
              <DialogFooter>
                <DialogClose asChild><Button variant="outline">{t('stopPayments.cancel')}</Button></DialogClose>
        ...
        <Card>
          ...
          <CardContent>
            {stopPayments.length === 0 ? (
              <EmptyState icon={Ban} title={t('stopPayments.noStopPayments')} description={t('stopPayments.noStopPaymentsDesc')} />
            ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-sm">
                            Check #{sp.checkNumberStart}{sp.checkNumberEnd ? ` - #${sp.checkNumberEnd}` : ""}
                          </p>
                          <p className="text-xs text-muted-foreground">{t('stopPayments.accountLabel')} {sp.accountMasked}</p>
                          {sp.payeeName && <p className="text-xs text-muted-foreground">{t('stopPayments.payeeLabel')} {sp.payeeName}</p>}
                          {sp.amountCents != null && <p className="text-xs text-muted-foreground">{t('stopPayments.amountLabel')} {formatCurrency(sp.amountCents)}</p>}
                          <p className="text-xs text-muted-foreground flex items-center gap-1">
                            <Clock className="w-3 h-3" />
                            {t(DURATION_LABEL_KEYS[sp.duration])}
                            {sp.expirationDate && ` - ${t('stopPayments.expires')} ${new Date(sp.expirationDate).toLocaleDateString()}`}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        {sp.status === "active" && (
                          <Button variant="ghost" size="icon" onClick={() => setCancelId(sp.id)} title={t('stopPayments.cancel')}>
                            <XCircle className="w-4 h-4 text-destructive" />
                          </Button>
                        )}
                        {(sp.status === "active" || sp.status === "expired") && (
                          <Button variant="ghost" size="icon" onClick={() => { setRenewId(sp.id); setRenewDuration("6months"); }} title={t('stopPayments.renew')}>
      ...
      <AlertDialog open={!!cancelId} onOpenChange={() => setCancelId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t('stopPayments.cancelStopPaymentDesc')}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t('stopPayments.keep')}</AlertDialogCancel>
            <AlertDialogAction onClick={() => cancelId && handleCancel(cancelId)}>{t('stopPayments.releaseStopPayment')}</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <Dialog open={!!renewId} onOpenChange={() => setRenewId(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader><DialogTitle>{t('stopPayments.renewStopPayment')}</DialogTitle></DialogHeader>
          <div className="space-y-4 py-2">
            <div>
              <Label>{t('stopPayments.newDuration')}</Label>
              <Select value={renewDuration} onValueChange={(v) => setRenewDuration(v as "6months" | "12months" | "permanent")}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="6months">{t('stopPayments.sixMonths')}</SelectItem>
                  <SelectItem value="12months">{t('stopPayments.twelveMonths')}</SelectItem>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRenewId(null)}>{t('stopPayments.cancel')}</Button>
