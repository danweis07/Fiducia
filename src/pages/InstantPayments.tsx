...
const STATUS_STYLES: Record<InstantPaymentStatus, { variant: "default" | "secondary" | "destructive" | "outline"; label: string }> = {
...
export default function InstantPayments() {
  ...
  const railLimit = preferredRail === "fednow"
  ...
  async function handleSend() {
    try {
      ...
      toast({ title: "Payment sent", description: `${formatCurrency(amountCents)} sent via ${RAIL_LABELS[preferredRail]}` });
      resetForm();
      setTab("history");
    } catch (err) {
      handleError(err);
  ...
  async function handleExportISO20022(paymentId: string) {
    try {
      ...
      toast({ title: "ISO 20022 exported", description: `${result.messageType} message downloaded` });
    } catch (err) {
      handleError(err);
    }
  ...
  const canSubmit = fromAccountId && receiverName && receiverRouting.length === 9 &&
    receiverAccount && amountCents > 0 && amountCents <= railLimit && description;

  if (accountsLoading) return <AppShell><PageSkeleton /></AppShell>;
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as "send" | "history")}>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select value={preferredRail} onValueChange={(v) => setPreferredRail(v as InstantPaymentRail)}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input value={receiverRouting} onChange={(e) => setReceiverRouting(e.target.value.replace(/\D/g, "").slice(0, 9))} placeholder="021000089" maxLength={9} />
                      {receiverRouting.length === 9 && receiverCheck && (
                        <p className={`text-xs ${receiverCheck.eligible ? "text-green-600" : "text-red-600"}`}>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input value={receiverAccount} onChange={(e) => setReceiverAccount(e.target.value)} placeholder="Account number" />
                    </div>
                    <div className="space-y-2">
                      <Label>Amount ($)</Label>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Payment description" rows={2} />
                  </div>

                  <Button onClick={() => setStep("confirm")} disabled={!canSubmit} className="w-full">
                    Review Payment
                  </Button>
                </CardContent>
              </Card>
            ...
            {step === "confirm" && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Review before sending — instant payments cannot be reversed</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <Button onClick={handleSend} disabled={sendPayment.isPending} className="flex-1">
          ...
          <TabsContent value="history" className="space-y-4">
            {paymentsLoading ? (
              ...
              <EmptyState icon={Zap} title="No instant payments yet" description="Send your first real-time payment to get started" />
            ) : (
                    ...
                    <Card key={p.paymentId}>
                      <CardContent className="flex items-center justify-between p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">{isOutbound ? p.receiverName : p.senderName}</p>
                            <p className="text-xs text-muted-foreground">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            <p className={`font-semibold ${isOutbound ? "text-red-600" : "text-green-600"}`}>
                              {isOutbound ? "-" : "+"}{formatCurrency(p.amountCents)}
                            </p>
      ...
      <AlertDialog open={!!exportPaymentId} onOpenChange={() => setExportPaymentId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Download the ISO 20022 pacs.008 XML message for this payment.
              This is the standard format used by FedNow, SWIFT, and international clearinghouses.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => exportPaymentId && handleExportISO20022(exportPaymentId)}>
