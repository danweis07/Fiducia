...
const featureKeys = [
  ...
  { icon: Smartphone, titleKey: 'home.features.mobileBanking', descKey: 'home.features.mobileBankingDesc' },
  { icon: Zap, titleKey: 'home.features.instantTransfers', descKey: 'home.features.instantTransfersDesc' },
  { icon: PiggyBank, titleKey: 'home.features.competitiveRates', descKey: 'home.features.competitiveRatesDesc' },
  { icon: CreditCard, titleKey: 'home.features.rewardsCards', descKey: 'home.features.rewardsCardsDesc' },
...
const testimonials = [
  {
    ...
    quote: 'Switching to this credit union was the best financial decision I\'ve made. The savings rates are unbeatable and the mobile app is fantastic.',
    rating: 5,
  },
  {
    name: 'James T.',
    location: 'Cherry Hill, NJ',
    quote: 'Got my auto loan approved in under an hour with an incredible rate. The staff at the Cherry Hill branch went above and beyond.',
    rating: 5,
  },
  {
    name: 'Priya K.',
    location: 'Wilmington, DE',
    quote: 'I love that my credit union is not-for-profit. Lower fees, better rates, and they actually care about the community.',
    rating: 5,
  },
];
    ...
    <PublicShell tenantName="Demo Credit Union">
      ...
      <StructuredData
        ...
        data={{
          ...
          description: 'Not-for-profit financial cooperative serving communities nationwide since 1952.',
            ...
            <div>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link to="/open-account">
                  <Button size="lg" className="bg-white text-slate-900 hover:bg-blue-50 gap-2 text-base">
              ...
              <div className="mt-8 flex items-center gap-4 text-sm text-blue-200">
                <span className="flex items-center gap-1"><CheckCircle2 className="h-4 w-4" /> {t('home.noMonthlyFees')}</span>
                <span className="flex items-center gap-1"><CheckCircle2 className="h-4 w-4" /> {t('home.ncuaInsured')}</span>
                <span className="flex items-center gap-1"><CheckCircle2 className="h-4 w-4" /> {t('home.freeAtms')}</span>
              </div>
            </div>
              ...
              <Card className="bg-white/10 backdrop-blur-sm border-white/20 text-white">
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    {rates.map((rate) => (
                      <div key={rate.product} className="flex items-center justify-between py-2 border-b border-white/10 last:border-0">
                        <div className="flex items-center gap-2">
                          <span className="text-sm text-blue-100">{rate.product}</span>
                          {rate.badge && (
                            <Badge variant="secondary" className="bg-blue-500/30 text-blue-200 text-[10px] px-1.5">
                              {rate.badge}
                            </Badge>
                          )}
                        </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              { icon: CreditCard, labelKey: 'home.products.checking', href: '/checking', color: 'bg-blue-50 text-blue-600' },
              { icon: PiggyBank, labelKey: 'home.products.savingsCds', href: '/savings', color: 'bg-green-50 text-green-600' },
              { icon: CreditCard, labelKey: 'home.products.creditCards', href: '/credit-cards', color: 'bg-purple-50 text-purple-600' },
              { icon: Building2, labelKey: 'home.products.mortgages', href: '/mortgages', color: 'bg-amber-50 text-amber-600' },
              { icon: Zap, labelKey: 'home.products.autoLoans', href: '/auto-loans', color: 'bg-red-50 text-red-600' },
              { icon: Users, labelKey: 'home.products.personalLoans', href: '/loans', color: 'bg-teal-50 text-teal-600' },
            ].map((item) => (
              <Link
                key={item.labelKey}
                to={item.href}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {featureKeys.map((feature) => (
              <Card key={feature.titleKey} className="border-0 shadow-sm hover:shadow-md transition-shadow">
