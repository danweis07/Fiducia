  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t("loanApplication.loanDetailsDesc", { product: selectedProduct ? selectedProduct.name.toLowerCase() : t("loanApplication.loanGeneric") })}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
            ...
            <SelectContent>
              {termOptions.map((termVal) => (
                <SelectItem key={termVal} value={String(termVal)}>
                  {t("loanApplication.termOption", { months: termVal, years: (termVal / 12).toFixed(termVal % 12 === 0 ? 0 : 1) })}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        {selectedProduct && (
          <div className="rounded-lg bg-muted/50 p-4 text-sm space-y-1">
            <p>
              <strong>{t("loanApplication.rate")}:</strong> {(selectedProduct.interestRateBps / 100).toFixed(2)}%{" "}
            ...
            {form.requestedAmountDollars && form.termMonths && (
              <p className="text-muted-foreground">
                {t("loanApplication.estimatedPaymentNote")}
              </p>
            )}
          </div>
        )}
