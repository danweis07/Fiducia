...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const resolvedMode: 'light' | 'dark' =
    theme.mode === 'system' ? systemMode : theme.mode;
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--primary', isDark ? deriveDarkPrimary(theme.primaryColor) : theme.primaryColor);
    root.style.setProperty('--primary-foreground', isDark ? '220 20% 10%' : '0 0% 100%');
    root.style.setProperty('--secondary', theme.secondaryColor);
    root.style.setProperty('--accent', theme.accentColor);
    root.style.setProperty('--ring', isDark ? deriveDarkPrimary(theme.primaryColor) : theme.primaryColor);
