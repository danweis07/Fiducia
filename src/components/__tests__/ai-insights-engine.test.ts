...
describe('checkOverdraftRisk', () => {
  it('returns null when there are no checking accounts', () => {
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'savings', balanceCents: 500000, availableBalanceCents: 500000 }],
      recurringPayments: [{ amountCents: 10000, nextDate: daysFromNow(2), payeeName: 'Electric Co' }],
    });
  ...
  it('returns null when no upcoming payments are within 7 days', () => {
    const ctx = makeContext({
      ...
      recurringPayments: [
        { amountCents: 100000, nextDate: daysFromNow(14), payeeName: 'Rent' },
      ],
    });
    expect(checkOverdraftRisk(ctx)).toBeNull();
  });
  ...
  it('returns null when balance covers all upcoming payments', () => {
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 200000, availableBalanceCents: 200000 }],
      recurringPayments: [
  ...
  it('returns critical insight when shortfall > 50% of upcoming total', () => {
    ...
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 10000, availableBalanceCents: 10000 }],
      recurringPayments: [
        { amountCents: 50000, nextDate: daysFromNow(2), payeeName: 'Electric' },
      ],
    });
  ...
  it('returns warning when shortfall <= 50% of upcoming total', () => {
    ...
    const ctx = makeContext({
      ...
      recurringPayments: [
        { amountCents: 50000, nextDate: daysFromNow(3), payeeName: 'Rent' },
      ],
    });
    const insight = checkOverdraftRisk(ctx);
    expect(insight).not.toBeNull();
  ...
  it('includes a suggested transfer action', () => {
    const ctx = makeContext({
      ...
      recurringPayments: [
        { amountCents: 20000, nextDate: daysFromNow(1), payeeName: 'Insurance' },
      ],
    });
    const insight = checkOverdraftRisk(ctx);
    expect(insight).not.toBeNull();
  ...
  it('sums available balances across multiple checking accounts', () => {
    ...
    const ctx = makeContext({
      ...
      recurringPayments: [
        { amountCents: 30000, nextDate: daysFromNow(2), payeeName: 'Utilities' },
      ],
    });
    const insight = checkOverdraftRisk(ctx);
    expect(insight).not.toBeNull();
...
describe('checkSpendingAnomaly', () => {
  ...
  it('returns null with only credit transactions', () => {
    const ctx = makeContext({
      recentTransactions: [
        { amountCents: 100000, category: 'income', merchantName: 'Employer', date: daysAgo(1), type: 'credit' },
      ],
    });
    expect(checkSpendingAnomaly(ctx)).toBeNull();
  });
  ...
  it('returns null when current spending is not 2x the average', () => {
    ...
    const ctx = makeContext({
      recentTransactions: [
        { amountCents: -10000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(1), type: 'debit' },
        { amountCents: -8000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(8), type: 'debit' },
        { amountCents: -8000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(15), type: 'debit' },
        { amountCents: -8000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(22), type: 'debit' },
        { amountCents: -8000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(28), type: 'debit' },
      ],
    });
    expect(checkSpendingAnomaly(ctx)).toBeNull();
  });
  ...
  it('detects anomaly when current week spending > 2x average', () => {
    ...
    const ctx = makeContext({
      recentTransactions: [
        { amountCents: -50000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(0), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(7), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(14), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(21), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'Restaurant', date: daysAgo(28), type: 'debit' },
      ],
    });
  ...
  it('returns the most severe anomaly across categories', () => {
    ...
    const ctx = makeContext({
      recentTransactions: [
        ...
        { amountCents: -30000, category: 'dining', merchantName: 'R', date: daysAgo(0), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(8), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(15), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(22), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(28), type: 'debit' },
        // groceries: week 0 = $500, weeks 1-4 = $100 each => 5x
        { amountCents: -50000, category: 'groceries', merchantName: 'G', date: daysAgo(0), type: 'debit' },
        { amountCents: -10000, category: 'groceries', merchantName: 'G', date: daysAgo(8), type: 'debit' },
        { amountCents: -10000, category: 'groceries', merchantName: 'G', date: daysAgo(15), type: 'debit' },
        { amountCents: -10000, category: 'groceries', merchantName: 'G', date: daysAgo(22), type: 'debit' },
        { amountCents: -10000, category: 'groceries', merchantName: 'G', date: daysAgo(28), type: 'debit' },
      ],
    });
    const insight = checkSpendingAnomaly(ctx);
    expect(insight).not.toBeNull();
  ...
  it('severity is info when ratio >= 2 but < 3', () => {
    ...
    const ctx = makeContext({
      recentTransactions: [
        { amountCents: -20000, category: 'dining', merchantName: 'R', date: daysAgo(0), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(8), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(15), type: 'debit' },
        { amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(22), type: 'debit' },
      ],
...
describe('checkSavingsOpportunity', () => {
  it('returns null when there are no checking accounts', () => {
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'savings', balanceCents: 1000000, availableBalanceCents: 1000000 }],
    });
  ...
  it('returns null when balance is insufficient (excess < $2,000)', () => {
    ...
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 300000, availableBalanceCents: 300000 }],
  ...
  it('returns null when fewer than 14 days of transaction data', () => {
    ...
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 5000000, availableBalanceCents: 5000000 }],
      recentTransactions: transactions,
    });
    expect(checkSavingsOpportunity(ctx)).toBeNull();
  });
  ...
  it('returns positive insight when excess > $2,000', () => {
    ...
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 1000000, availableBalanceCents: 1000000 }],
      recentTransactions: transactions,
    });
...
describe('checkPaymentFailureRisk', () => {
  ...
  it('returns null when projected balance covers all payments', () => {
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 100000, availableBalanceCents: 100000 }],
      recurringPayments: [
  ...
  it('returns critical when payment is less than 2 days away', () => {
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 5000, availableBalanceCents: 5000 }],
      recurringPayments: [
        { amountCents: 20000, nextDate: daysFromNow(1), payeeName: 'Electric' },
      ],
    });
  ...
  it('returns warning when payment is more than 2 days away', () => {
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 5000, availableBalanceCents: 5000 }],
      recurringPayments: [
        { amountCents: 20000, nextDate: daysFromNow(5), payeeName: 'Electric' },
      ],
    });
    const insight = checkPaymentFailureRisk(ctx);
    expect(insight).not.toBeNull();
...
describe('generateInsights', () => {
  it('returns empty array when no conditions are met', () => {
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 500000, availableBalanceCents: 500000 }],
    });
  ...
  it('runs all checks and collects results', () => {
    ...
    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 5000, availableBalanceCents: 5000 }],
      recurringPayments: [
        { amountCents: 50000, nextDate: daysFromNow(2), payeeName: 'Rent' },
      ],
    });
    const insights = generateInsights(ctx);
    ...
    expect(
      types.includes('overdraft_prediction') || types.includes('payment_failure_risk'),
    ).toBe(true);
  });
  ...
  it('sorts by severity: critical first, then warning, info, positive', () => {
    ...
    transactions.push({ amountCents: -20000, category: 'dining', merchantName: 'R', date: daysAgo(0), type: 'debit' });
    transactions.push({ amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(8), type: 'debit' });
    transactions.push({ amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(15), type: 'debit' });
    transactions.push({ amountCents: -10000, category: 'dining', merchantName: 'R', date: daysAgo(22), type: 'debit' });

    const ctx = makeContext({
      accounts: [{ id: 'a1', type: 'checking', balanceCents: 5000, availableBalanceCents: 5000 }],
      recurringPayments: [
        { amountCents: 50000, nextDate: daysFromNow(1), payeeName: 'Rent' },
      ],
      recentTransactions: transactions,
    });
