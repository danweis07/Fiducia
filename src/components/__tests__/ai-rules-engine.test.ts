...
describe('evaluateRule — transaction', () => {
  it('fires when transaction matches type and amount filters', () => {
    const ctx = makeRuleCtx({
      trigger: {
        type: 'transaction',
        event: { transactionType: 'debit', amountCents: 75000, merchantName: 'Amazon', transactionId: 'tx-1' },
      },
      rule: {
        id: 'r1',
        triggerType: 'transaction',
...
describe('parseNaturalLanguageRule', () => {
  ...
  it('throws on empty input', async () => {
    ...
    await expect(parseNaturalLanguageRule('', adapter)).rejects.toThrow('Rule input cannot be empty');
    await expect(parseNaturalLanguageRule('   ', adapter)).rejects.toThrow('Rule input cannot be empty');
  });
  ...
  it('throws on input exceeding max length', async () => {
    ...
    await expect(parseNaturalLanguageRule(longInput, adapter)).rejects.toThrow('exceeds maximum length');
  });
  ...
  it('throws when parsed rule has invalid trigger type', async () => {
    ...
    const adapter = makeMockAdapter(mockParsed);
    await expect(
      parseNaturalLanguageRule('Do something weird', adapter),
  ...
  it('throws when parsed rule has invalid action type', async () => {
    ...
    const adapter = makeMockAdapter(mockParsed);
    await expect(
      parseNaturalLanguageRule('Do something', adapter),
  ...
  it('throws when balance_threshold rule has invalid config', async () => {
    ...
    const adapter = makeMockAdapter(mockParsed);
    await expect(
      parseNaturalLanguageRule('When balance drops', adapter),
    ).rejects.toThrow('non-negative thresholdCents');
  });
  ...
  it('throws when schedule rule is missing cron expression', async () => {
    ...
    const adapter = makeMockAdapter(mockParsed);
    await expect(
      parseNaturalLanguageRule('Every month', adapter),
...
describe('RULE_EXAMPLES', () => {
  ...
  it('each example has valid trigger and action types', () => {
    const validTriggers: TriggerType[] = ['transaction', 'balance_threshold', 'schedule', 'direct_deposit', 'recurring_payment'];
