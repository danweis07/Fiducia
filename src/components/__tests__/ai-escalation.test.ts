...
describe('analyzeEscalation', () => {
  ...
  it('throws when transcript is empty', async () => {
    ...
    await expect(analyzeEscalation(ctx, adapter)).rejects.toThrow('at least one transcript message');
  });
  ...
  it('falls back to heuristic category when AI returns invalid category', async () => {
    ...
    const ctx = makeEscalationCtx({
      transcript: [
        { role: 'user', content: 'I need to dispute a charge at Amazon for $50.' },
      ],
    });
  ...
  it('includes preFilled data extracted from transcript', async () => {
    ...
    const ctx = makeEscalationCtx({
      transcript: [
        { role: 'user', content: 'I want to dispute a $45.00 charge from Starbucks on January 15.' },
      ],
    });
