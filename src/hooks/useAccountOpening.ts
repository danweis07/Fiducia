...
export function useCreateApplication() {
  ...
  return useMutation({
    mutationFn: (applicant: Record<string, unknown>) =>
      gateway.accountOpening.create(applicant),
    onSuccess: () => {
...
export function useCompleteApplication() {
  ...
  return useMutation({
    mutationFn: (applicationId: string) =>
      gateway.accountOpening.complete(applicationId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: accountOpeningKeys.all });
    },
  });
...
export function useCancelApplication() {
  ...
  return useMutation({
    mutationFn: (applicationId: string) =>
      gateway.accountOpening.cancel(applicationId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: accountOpeningKeys.all });
    },
  });
