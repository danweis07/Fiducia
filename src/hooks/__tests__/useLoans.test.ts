...
import { useLoans, useLoan, useLoanSchedule, useLoanPayments, useMakeLoanPayment, loanKeys } from '../useLoans';
...
const mockLoans = [
  { id: 'loan-1', loanType: 'auto', loanNumberMasked: '****5678', balanceCents: 2450000, principalCents: 2500000, interestRateBps: 549, status: 'active' },
  { id: 'loan-2', loanType: 'mortgage', loanNumberMasked: '****9012', balanceCents: 28500000, principalCents: 30000000, interestRateBps: 625, status: 'active' },
];
...
describe('useLoans', () => {
  ...
  it('passes status filter', async () => {
    ...
    const { result } = renderHook(() => useLoans({ status: 'active' }), { wrapper: createWrapper() });
...
describe('useLoanSchedule', () => {
  ...
  it('passes pagination params', async () => {
    ...
    const { result } = renderHook(
      () => useLoanSchedule('loan-1', { limit: 12 }),
