  ...
  it('calls gateway addOwner', async () => {
    ...
    await act(async () => {
      result.current.mutate({
        accountId: 'acct-1', email: 'bob@test.com', firstName: 'Bob',
        lastName: 'Smith', relationship: 'spouse' as Record<string, unknown>, permissions: 'full' as Record<string, unknown>,
      });
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
...
describe('useAcceptInvitation', () => {
  ...
  it('calls gateway acceptInvitation', async () => {
    ...
    await act(async () => { result.current.mutate('inv-1'); });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
});
