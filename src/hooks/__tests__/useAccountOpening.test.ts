  ...
  gateway: {
    accountOpening: {
      config: vi.fn().mockResolvedValue({
        products: [
          { id: 'prod-checking', type: 'checking', name: 'Free Checking', apyBps: 10, minOpeningDepositCents: 2500, monthlyFeeCents: 0, isAvailable: true },
          { id: 'prod-savings', type: 'savings', name: 'High-Yield Savings', apyBps: 425, minOpeningDepositCents: 10000, monthlyFeeCents: 0, isAvailable: true },
        ],
      ...
      complete: vi.fn().mockResolvedValue({
        id: 'app-001',
        status: 'completed',
        createdAccounts: [{ accountId: 'acct-new', accountNumberMasked: '****9876', type: 'checking' }],
      }),
...
describe('useSelectProducts', () => {
  it('should select products for application', async () => {
    ...
    expect(gateway.accountOpening.selectProducts).toHaveBeenCalledWith('app-001', ['prod-checking']);
  });
});
