...
vi.mock('@/lib/gateway', () => ({
  gateway: {
    directDeposit: {
      employers: vi.fn(), status: vi.fn(), list: vi.fn(),
      initiate: vi.fn(), cancel: vi.fn(), confirm: vi.fn(),
    },
  },
}));
...
import {
  directDepositKeys, useEmployers, useSwitchStatus,
  useInitiateSwitch, useCancelSwitch, useConfirmSwitch,
...
describe('useEmployers', () => {
  ...
  it('passes search params', async () => {
    ...
    await waitFor(() => expect(gateway.directDeposit.employers).toHaveBeenCalledWith({ query: 'Acme', limit: 10 }));
  });
});
...
describe('useSwitchStatus', () => {
  ...
  it('fetches status for switch', async () => {
    vi.mocked(gateway.directDeposit.status).mockResolvedValue({ switch: { id: 's-1', status: 'completed' } });
  ...
  it('initiates a switch', async () => {
    ...
    await act(async () => {
      result.current.mutate({
        accountId: 'acct-1', employerId: 'e-1', allocationType: 'full' as Record<string, unknown>,
      });
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
...
describe('useCancelSwitch', () => {
  ...
  it('cancels a switch', async () => {
    ...
    await act(async () => { result.current.mutate('s-1'); });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
});
