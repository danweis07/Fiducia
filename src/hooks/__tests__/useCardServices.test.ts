...
describe('useTravelNotices', () => {
  ...
  it('fetches travel notices', async () => {
    const mockNotices = [{ id: 'tn-1', destinations: [{ country: 'FR' }], startDate: '2026-06-01' }];
...
describe('useCardReplacementStatus', () => {
  ...
  it('fetches replacement status by id', async () => {
    vi.mocked(gateway.cardReplacements.status).mockResolvedValue({ id: 'cr-1', status: 'shipped', trackingNumber: 'TRK123' });

    const { result } = renderHook(() => useCardReplacementStatus('cr-1'), { wrapper: createWrapper() });
