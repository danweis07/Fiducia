...
const mockTransactions = [
  { id: 'txn-1', description: 'Grocery Store', amountCents: -8500, category: 'groceries', status: 'posted', transactionDate: '2026-03-10' },
  { id: 'txn-2', description: 'Direct Deposit', amountCents: 300000, category: 'income', status: 'posted', transactionDate: '2026-03-09' },
  { id: 'txn-3', description: 'Coffee Shop', amountCents: -450, category: 'dining', status: 'pending', transactionDate: '2026-03-11' },
];
...
describe('useTransactions', () => {
  ...
  it('passes accountId filter', async () => {
    ...
    const { result } = renderHook(
      () => useTransactions({ accountId: 'acct-1' }),
  ...
  it('passes category filter', async () => {
    ...
    const { result } = renderHook(
      () => useTransactions({ category: 'groceries' as never }),
      { wrapper: createWrapper() },
    );
  ...
  it('passes pagination parameters', async () => {
    ...
    const { result } = renderHook(
      () => useTransactions({ limit: 10, offset: 20 }),
      { wrapper: createWrapper() },
    );
  ...
  it('passes status filter', async () => {
    ...
    const { result } = renderHook(
      () => useTransactions({ status: 'pending' as never }),
      { wrapper: createWrapper() },
    );
  ...
  it('passes type filter', async () => {
    ...
    const { result } = renderHook(
      () => useTransactions({ type: 'debit' as never }),
      { wrapper: createWrapper() },
    );
...
describe('useTransaction', () => {
  ...
  it('fetches single transaction', async () => {
    vi.mocked(gateway.transactions.get).mockResolvedValue({
      transaction: { id: 'txn-1', description: 'Grocery Store', amountCents: -8500, category: 'groceries', status: 'posted' },
    } as never);
  ...
  it('returns credit transaction data', async () => {
    vi.mocked(gateway.transactions.get).mockResolvedValue({
      transaction: { id: 'txn-2', description: 'Direct Deposit', amountCents: 300000, category: 'income', status: 'posted' },
    } as never);
  ...
  it('returns pending transaction data', async () => {
    vi.mocked(gateway.transactions.get).mockResolvedValue({
      transaction: { id: 'txn-3', description: 'Pending Auth', amountCents: -1500, status: 'pending' },
    } as never);
