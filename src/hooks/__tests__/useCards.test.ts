...
const mockCards = [
  { id: 'card-1', cardNumberMasked: '****4242', type: 'debit', status: 'active', dailyLimitCents: 200000 },
  { id: 'card-2', cardNumberMasked: '****5353', type: 'credit', status: 'active', dailyLimitCents: 500000 },
  { id: 'card-3', cardNumberMasked: '****7878', type: 'debit', status: 'locked', dailyLimitCents: 100000 },
];
...
describe('useCards', () => {
  ...
  it('includes locked cards in results', async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    const locked = result.current.data?.cards.filter((c: { status: string }) => c.status === 'locked');
...
describe('useLockCard', () => {
  ...
  it('locks a card', async () => {
    vi.mocked(gateway.cards.lock).mockResolvedValue({ card: { id: 'card-1', status: 'locked' } } as never);
...
describe('useUnlockCard', () => {
  ...
  it('unlocks a card', async () => {
    vi.mocked(gateway.cards.unlock).mockResolvedValue({ card: { id: 'card-3', status: 'active' } } as never);
...
describe('useSetCardLimit', () => {
  ...
  it('sets card daily limit', async () => {
    vi.mocked(gateway.cards.setLimit).mockResolvedValue({ card: { id: 'card-1', dailyLimitCents: 300000 } } as never);
  ...
  it('sets very low limit', async () => {
    vi.mocked(gateway.cards.setLimit).mockResolvedValue({ card: { id: 'card-1', dailyLimitCents: 100 } } as never);
  ...
  it('sets high limit', async () => {
    vi.mocked(gateway.cards.setLimit).mockResolvedValue({ card: { id: 'card-2', dailyLimitCents: 10000000 } } as never);
  ...
  it('sets zero limit (effectively disabling card usage)', async () => {
    vi.mocked(gateway.cards.setLimit).mockResolvedValue({ card: { id: 'card-1', dailyLimitCents: 0 } } as never);
