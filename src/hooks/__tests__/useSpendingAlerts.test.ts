...
vi.mock('@/lib/gateway', () => ({
  gateway: {
    spendingAlerts: {
      ...
      delete: vi.fn(), history: vi.fn(), summary: vi.fn(),
    },
  },
}));
...
import {
  useSpendingAlerts, useCreateAlert, useUpdateAlert,
  useDeleteAlert, useAlertHistory, useAlertSummary,
  ...
  it('creates an alert', async () => {
    ...
    await act(async () => {
      result.current.mutate({
        name: 'High Spend', alertType: 'threshold' as Record<string, unknown>,
        thresholdCents: 50000, channels: ['push', 'email'],
      });
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
...
describe('useDeleteAlert', () => {
  ...
  it('deletes an alert', async () => {
    ...
    await act(async () => { result.current.mutate('a-1'); });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
});
...
describe('useAlertSummary', () => {
  ...
  it('fetches summary', async () => {
    vi.mocked(gateway.spendingAlerts.summary).mockResolvedValue({ activeAlerts: 3, triggeredToday: 1 });
