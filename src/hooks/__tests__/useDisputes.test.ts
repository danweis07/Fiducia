...
vi.mock('@/lib/gateway', () => ({
  gateway: {
    disputes: {
      list: vi.fn(), get: vi.fn(), file: vi.fn(),
      addDocument: vi.fn(), cancel: vi.fn(), timeline: vi.fn(),
    },
  },
}));
...
import {
  disputeKeys, useDisputes, useDispute, useFileDispute,
  useCancelDispute, useDisputeTimeline,
} from '../useDisputes';
  ...
  it('files a dispute', async () => {
    ...
    await act(async () => {
      result.current.mutate({
        transactionId: 'txn-1', reason: 'unauthorized', description: 'Not my charge',
      });
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
