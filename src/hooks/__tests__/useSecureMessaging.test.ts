...
vi.mock('@/lib/gateway', () => ({
  gateway: {
    messaging: {
      ...
      reply: vi.fn(), markRead: vi.fn(), archive: vi.fn(),
      listDepartments: vi.fn(), unreadCount: vi.fn(),
    },
  },
}));
...
import {
  messagingKeys, useMessageThreads, useThread, useCreateThread,
  useReplyToThread, useMarkThreadRead, useArchiveThread,
  useMessageDepartments, useUnreadMessageCount,
...
describe('useMessageDepartments', () => {
  ...
  it('fetches departments', async () => {
    vi.mocked(gateway.messaging.listDepartments).mockResolvedValue({ departments: [{ id: 'd-1', name: 'Support' }] });
