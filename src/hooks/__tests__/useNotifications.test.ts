...
import { useNotifications, useUnreadCount, useMarkRead, useMarkAllRead, notificationKeys } from '../useNotifications';
...
const mockNotifications = [
  { id: 'n-1', title: 'Transfer Complete', message: 'Your $500 transfer is done', read: false, createdAt: '2026-03-14T10:00:00Z' },
  { id: 'n-2', title: 'Low Balance Alert', message: 'Checking below $100', read: false, createdAt: '2026-03-13T08:00:00Z' },
  { id: 'n-3', title: 'Statement Ready', message: 'February statement available', read: true, createdAt: '2026-03-01T12:00:00Z' },
];
...
describe('notificationKeys', () => {
  ...
  it('has correct list key', () => {
    expect(notificationKeys.list({ unreadOnly: true })).toEqual(['notifications', 'list', { unreadOnly: true }]);
  });
...
describe('useNotifications', () => {
  ...
  it('passes unreadOnly filter', async () => {
    vi.mocked(gateway.notifications.list).mockResolvedValue({ notifications: [mockNotifications[0], mockNotifications[1]] });
  ...
  it('passes pagination params', async () => {
    ...
    const { result } = renderHook(
      () => useNotifications({ limit: 10, offset: 5 }),
      { wrapper: createWrapper() },
    );
...
describe('useMarkRead', () => {
  ...
  it('handles mark read failure', async () => {
    vi.mocked(gateway.notifications.markRead).mockRejectedValue(new Error('Notification not found'));
