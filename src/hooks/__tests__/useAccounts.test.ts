...
describe('useAccounts', () => {
  ...
  it('fetches accounts successfully', async () => {
    const mockAccounts = [
      { id: 'acct-1', type: 'checking', balanceCents: 150000, status: 'active', nickname: 'Main Checking', accountNumberMasked: '****1234' },
      { id: 'acct-2', type: 'savings', balanceCents: 500000, status: 'active', nickname: 'Emergency Fund', accountNumberMasked: '****5678' },
    ];
  ...
  it('includes all account types', async () => {
    const mockAccounts = [
      { id: '1', type: 'checking', balanceCents: 100000, status: 'active', accountNumberMasked: '****0001' },
      { id: '2', type: 'savings', balanceCents: 200000, status: 'active', accountNumberMasked: '****0002' },
      { id: '3', type: 'cd', balanceCents: 1000000, status: 'active', accountNumberMasked: '****0003' },
      { id: '4', type: 'money_market', balanceCents: 500000, status: 'active', accountNumberMasked: '****0004' },
    ];
    vi.mocked(gateway.accounts.list).mockResolvedValue({ accounts: mockAccounts });
...
describe('useAccount', () => {
  ...
  it('fetches single account by id', async () => {
    vi.mocked(gateway.accounts.get).mockResolvedValue({
      account: { id: 'acct-1', type: 'checking', balanceCents: 524300, availableBalanceCents: 500000, status: 'active', interestRateBps: 10 },
    } as never);
