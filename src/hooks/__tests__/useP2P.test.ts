...
vi.mock('@/lib/gateway', () => ({
  gateway: {
    p2p: {
      getEnrollment: vi.fn(), enroll: vi.fn(), unenroll: vi.fn(),
      send: vi.fn(), requestMoney: vi.fn(), listTransactions: vi.fn(),
      cancelRequest: vi.fn(), getLimits: vi.fn(),
    },
  },
}));
...
import {
  p2pKeys, useP2PEnrollment, useEnrollP2P,
...
describe('useEnrollP2P', () => {
  ...
  it('enrolls user in P2P', async () => {
    ...
    await act(async () => {
      result.current.mutate({ accountId: 'acct-1', enrollmentType: 'email', enrollmentValue: 'test@test.com' });
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
});
...
describe('useSendP2P', () => {
  ...
  it('sends money via P2P', async () => {
    ...
    await act(async () => {
      result.current.mutate({ recipientType: 'email', recipientValue: 'bob@test.com', amountCents: 5000 });
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
});
...
describe('useRequestP2P', () => {
  ...
  it('requests money', async () => {
    ...
    await act(async () => {
      result.current.mutate({ recipientType: 'email', recipientValue: 'alice@test.com', amountCents: 2500 });
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
});
...
describe('useCancelP2PRequest', () => {
  ...
  it('cancels a request', async () => {
    ...
    await act(async () => { result.current.mutate('req-1'); });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
});
