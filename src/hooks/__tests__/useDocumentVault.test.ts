...
vi.mock('@/lib/gateway', () => ({
  gateway: {
    vault: {
      list: vi.fn(), upload: vi.fn(), get: vi.fn(),
      update: vi.fn(), delete: vi.fn(), summary: vi.fn(), search: vi.fn(),
    },
  },
}));
...
import {
  useVaultDocuments, useUploadDocument, useVaultDocument,
  useDeleteVaultDocument, useVaultSummary,
...
describe('useDeleteVaultDocument', () => {
  ...
  it('deletes a document', async () => {
    ...
    await act(async () => { result.current.mutate('doc-1'); });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
});
...
describe('useVaultSummary', () => {
  ...
  it('fetches summary', async () => {
    vi.mocked(gateway.vault.summary).mockResolvedValue({ totalDocuments: 12, totalSizeBytes: 50000 });
...
describe('useSearchVaultDocuments', () => {
  ...
  it('searches documents with query', async () => {
    ...
    const { result } = renderHook(
      () => useSearchVaultDocuments({ query: 'tax' }),
