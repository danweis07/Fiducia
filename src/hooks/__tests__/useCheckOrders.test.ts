...
vi.mock('@/lib/gateway', () => ({
  gateway: {
    checks: {
      styles: vi.fn(), config: vi.fn(), listOrders: vi.fn(),
      getOrder: vi.fn(), createOrder: vi.fn(), cancelOrder: vi.fn(),
    },
  },
}));
...
import {
  checkOrderKeys, useCheckStyles, useCheckOrderConfig, useCheckOrders,
  useCheckOrder, useCreateCheckOrder, useCancelCheckOrder,
} from '../useCheckOrders';
...
describe('useCheckStyles', () => {
  ...
  it('fetches check styles', async () => {
    vi.mocked(gateway.checks.styles).mockResolvedValue({ styles: [{ id: 's-1', name: 'Classic' }] });
  ...
  it('creates an order', async () => {
    ...
    await act(async () => {
      result.current.mutate({
        accountId: 'acct-1', styleId: 's-1', quantity: 100, shippingMethod: 'standard',
      });
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
...
describe('useCancelCheckOrder', () => {
  ...
  it('cancels an order', async () => {
    ...
    await act(async () => { result.current.mutate('ord-1'); });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
  });
});
