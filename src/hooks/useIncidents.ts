...
import type { IncidentSeverity, IncidentStatus, ChangeRequestStatus, RollbackType } from '@/types/incident';
...
export function useCreateIncident() {
  ...
  return useMutation({
    mutationFn: (params: { title: string; description?: string; severity: IncidentSeverity; detectionSource?: string; affectedServices?: string[] }) =>
...
export function useUpdateIncident() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (params: { incidentId: string; status?: IncidentStatus; assignedTo?: string; resolutionSummary?: string }) =>
...
export function useInitiateRollback() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (params: { incidentId: string; fromVersion: string; toVersion: string; rollbackType: RollbackType }) =>
...
export function useCreateChangeRequest() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (params: { title: string; description?: string; changeType: string; gitBranch?: string; prUrl?: string }) =>
