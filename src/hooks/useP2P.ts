...
export function useEnrollP2P() {
  ...
  return useMutation({
    mutationFn: (params: { accountId: string; enrollmentType: 'email' | 'phone'; enrollmentValue: string }) =>
...
export function useSendP2P() {
  ...
  return useMutation({
    mutationFn: (params: { recipientType: 'email' | 'phone' | 'token'; recipientValue: string; amountCents: number; memo?: string }) =>
