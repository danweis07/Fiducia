...
export const internationalPaymentKeys = {
  ...
  payments: (network?: PaymentNetwork, status?: string) => ['internationalPayments', 'payments', network, status] as const,
...
export function useFXQuote(fromCurrency: string, toCurrency: string, fromAmountCents?: number) {
  return useQuery({
    ...
    queryFn: () => gateway.internationalPayments.getFXQuote({ fromCurrency, toCurrency, fromAmountCents }),
    enabled: !!fromCurrency && !!toCurrency,
    refetchInterval: 30000,
  });
}

export function useInternationalPayments(params: { status?: string; limit?: number; offset?: number } = {}) {
  return useQuery({
...
export function useInternationalLoanApplications(params: { country?: string; status?: string } = {}) {
  return useQuery({
