...
export function useStopPayments(params: { status?: string; accountId?: string; limit?: number; offset?: number } = {}) {
  return useQuery({
...
export function useRenewStopPayment() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: { stopPaymentId: string; duration: '6months' | '12months' | 'permanent' }) =>
