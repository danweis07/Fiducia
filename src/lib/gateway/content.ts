...
export function createContentDomain(callGateway: CallGatewayFn) {
  return {
    cms: {
      ...
      async listContent(params: { status?: string; contentType?: string; channel?: string; limit?: number; offset?: number } = {}) {
        return callGateway<{ content: CMSContent[]; _pagination?: Pagination }>('cms.content.list', params);
      },
      async getContent(id: string) {
        return callGateway<{ content: CMSContent }>('cms.content.get', { id });
      },
      async createContent(input: { slug: string; title: string; body?: string; contentType?: string; channels?: string[]; metadata?: Record<string, unknown>; scheduledAt?: string; expiresAt?: string }) {
      ...
      async getContentVersions(contentId: string) {
        return callGateway<{ versions: CMSContentVersion[] }>('cms.content.versions', { contentId });
      },
      async getPublicContent(slug: string) {
        return callGateway<{ content: CMSContent }>('cms.content.public', { slug });
      },
      async listPublicContent(params: { contentType?: string; channel?: string; limit?: number; offset?: number } = {}) {
        return callGateway<{ content: CMSContent[]; _pagination?: Pagination }>('cms.content.publicList', params);
      },
    },
    ...
    cmsTokens: {
      ...
      async create(input: { name: string; scopes?: string[]; allowedChannels?: string[] | null; rateLimit?: number; expiresAt?: string | null }) {
    ...
    experiments: {
      ...
      async update(id: string, updates: { name?: string; description?: string; trafficPercent?: number }) {
      ...
      async track(params: { experimentId: string; variantId: string; eventType: ExperimentEventType; metadata?: Record<string, unknown> }) {
    ...
    exports: {
      async list(params: { limit?: number; offset?: number; status?: string; reportType?: string } = {}) {
        return callGateway<{ exports: import('@/types/admin').ExportRequest[] }>('exports.list', params);
      },
      async create(params: { reportType: string; format: string; dateRangeStart: string; dateRangeEnd: string; filters?: Record<string, unknown> }) {
        return callGateway<{ export: import('@/types/admin').ExportRequest }>('exports.create', params);
      },
      async get(exportId: string) {
        return callGateway<{ export: import('@/types/admin').ExportRequest }>('exports.get', { exportId });
      },
      async download(exportId: string) {
        return callGateway<{ fileUrl: string; format: string; fileName: string }>('exports.download', { exportId });
      },
      async delete(exportId: string) {
        return callGateway<{ success: boolean }>('exports.delete', { exportId });
      },
      async summary() {
        return callGateway<{ summary: import('@/types/admin').ReportSummary }>('exports.summary', {});
      },
    },

    reportTemplates: {
      async list() {
        return callGateway<{ templates: import('@/types/admin').ReportTemplate[] }>('reports.templates.list', {});
      },
      async create(params: { name: string; description?: string; reportType: string; defaultFormat: string; defaultFilters?: Record<string, unknown>; schedule?: Record<string, unknown> }) {
        return callGateway<{ template: import('@/types/admin').ReportTemplate }>('reports.templates.create', params);
      },
      async update(params: { templateId: string; name?: string; description?: string; defaultFormat?: string; defaultFilters?: Record<string, unknown>; schedule?: Record<string, unknown> }) {
        return callGateway<{ template: import('@/types/admin').ReportTemplate }>('reports.templates.update', params);
      },
    ...
    sdui: {
      ...
      async persona() {
        return callGateway<{ personaId: string; personaLabel: string; traits: Record<string, unknown> }>('sdui.persona', {});
      },
      personas: {
        async list() {
          return callGateway<Array<UserPersona & { id: string; firmId: string; isActive: boolean; createdAt: string; updatedAt: string }>>('sdui.personas.list', {});
        },
        async create(input: PersonaCreateInput) {
          return callGateway<UserPersona & { id: string }>('sdui.personas.create', input);
        },
        async update(id: string, updates: { label?: string; description?: string; rules?: PersonaRule[]; priority?: number; isActive?: boolean }) {
          return callGateway<UserPersona & { id: string }>('sdui.personas.update', { id, ...updates });
        },
