...
export function createIntegrationsDomain(callGateway: CallGatewayFn) {
  return {
    externalAccounts: {
      async linkToken(params: { clientName?: string; products?: string[] } = {}) {
        return callGateway<{ linkToken: string; expiration: string }>(
          'external-accounts.link-token', params as unknown as Record<string, unknown>
        );
      },

      async exchange(publicToken: string) {
        return callGateway<{ itemId: string; linkedAt: string }>(
          'external-accounts.exchange', { publicToken }
        );
      },

      async list() {
        return callGateway<{ accounts: Array<{
          accountId: string;
          itemId: string;
      ...
      async balances(accountId?: string) {
        return callGateway<{ balances: Array<{
          accountId: string;
          currentCents: number;
    ...
    locations: {
      async search(params: {
        ...
        return callGateway<{ locations: import('@/types').BranchLocation[] }>(
          'locations.search', params as unknown as Record<string, unknown>
        );
      },
    },
    ...
    cd: {
      ...
      async updateMaturityAction(accountId: string, maturityAction: string, maturityTransferAccountId?: string) {
        return callGateway<{ success: boolean }>('cd.updateMaturityAction', {
          accountId, maturityAction, maturityTransferAccountId,
        });
      },
    },
    ...
    charges: {
      async definitions(params: { appliesTo?: string } = {}) {
        return callGateway<{ chargeDefinitions: ChargeDefinition[] }>('charges.definitions', params);
      },

      async list(params: { accountId?: string; status?: string; limit?: number; offset?: number } = {}) {
    ...
    standingInstructions: {
      async list(params: { status?: string } = {}) {
        return callGateway<{ instructions: StandingInstruction[] }>('standingInstructions.list', params);
      },
      ...
      async create(input: {
        ...
        return callGateway<{ instruction: StandingInstruction }>('standingInstructions.create', input);
      },
      ...
      async update(id: string, updates: {
        ...
        return callGateway<{ instruction: StandingInstruction }>('standingInstructions.update', { id, ...updates });
      },
    },
    ...
    accountOpening: {
      async config() {
        return callGateway<{
          products: Array<{
            ...
            apyBps: number; minOpeningDepositCents: number; monthlyFeeCents: number;
            feeWaiverDescription?: string; termMonths?: number; isAvailable: boolean;
          }>;
      ...
      async create(applicant: Record<string, unknown>) {
        return callGateway<{
          ...
          applicant: { firstNameInitial: string; lastNameMasked: string; emailMasked: string; ssnMasked: string };
