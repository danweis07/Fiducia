...
export function createMemberDomain(callGateway: CallGatewayFn) {
  return {
    auth: {
      ...
      async updateProfile(updates: Partial<Pick<BankingUser, 'firstName' | 'lastName' | 'phone' | 'preferredLanguage' | 'timezone'>>) {
        return callGateway<{ user: BankingUser }>('auth.updateProfile', updates as Record<string, unknown>);
      },
    },
    ...
    member: {
      ...
      async updateAddress(id: string, updates: Partial<Pick<MemberAddress, 'type' | 'isPrimary' | 'line1' | 'line2' | 'city' | 'state' | 'zip' | 'country'>>) {
        return callGateway<{ address: MemberAddress }>('member.updateAddress', { id, ...updates } as Record<string, unknown>);
      },
    ...
    sessions: {
      ...
      async revokeAll(currentSessionId?: string) {
        return callGateway<{ success: boolean }>('sessions.revokeAll', { excludeCurrent: true, currentSessionId });
      },
      async activity() {
        return callGateway<{
    ...
    devices: {
      ...
      async activity(deviceId: string, params: { limit?: number; offset?: number } = {}) {
        return callGateway<{ activity: DeviceActivityEntry[] }>('devices.activity', { deviceId, ...params });
      },
    ...
    notificationPreferences: {
      async get() {
        return callGateway<{ preferences: { channels: Record<string, boolean>; categories: Record<string, { enabled: boolean; channels: string[] }> } }>('notifications.preferences.get', {});
      },
      async update(params: { channels?: Record<string, boolean>; categories?: Record<string, { enabled: boolean; channels: string[] }> }) {
        return callGateway<{ preferences: { channels: Record<string, boolean>; categories: Record<string, { enabled: boolean; channels: string[] }> } }>('notifications.preferences.update', params);
      },
      async test(channel: string) {
        return callGateway<{ sent: boolean; channel: string; message: string }>('notifications.test', { channel });
      },
    },
  };
}
