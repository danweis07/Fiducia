    ...
    loans: {
      ...
      async payments(loanId: string, params: { limit?: number; offset?: number } = {}) {
        return callGateway<{ payments: LoanPayment[]; _pagination?: Pagination }>(
          'loans.payments', { loanId, ...params }
        );
      },

      async makePayment(input: { loanId: string; amountCents: number; fromAccountId: string; extraPrincipalCents?: number }) {
        return callGateway<{ payment: LoanPayment }>('loans.makePayment', input);
      },
    },

    loanOrigination: {
      async getApplication(params: { applicationId: string; institutionId: string; environmentId?: string; productId?: string }) {
        return callGateway<{ application: LoanApplication }>('loanOrigination.application.get', params);
      },
      ...
      async createApplication(input: {
        ...
        return callGateway<{ application: LoanApplication }>('loanOrigination.application.create', input);
      },

      async getDocument(params: { documentId: string; institutionId: string; environmentId?: string; productId?: string }) {
        return callGateway<{ document: LoanOriginationDocument }>('loanOrigination.document.get', params);
      },
