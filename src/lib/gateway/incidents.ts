...
export function createIncidentsDomain(callGateway: CallGatewayFn) {
  return {
    incidents: {
      async list(params: { status?: IncidentStatus; severity?: IncidentSeverity; limit?: number; offset?: number } = {}) {
      ...
      async create(params: { title: string; description?: string; severity: IncidentSeverity; detectionSource?: string; affectedServices?: string[] }) {
        return callGateway<{ incident: Incident }>('incidents.create', params);
      },
      async update(params: { incidentId: string; status?: IncidentStatus; assignedTo?: string; resolutionSummary?: string }) {
        return callGateway<{ incident: Incident }>('incidents.update', params);
      },
      async addTimeline(params: { incidentId: string; action: string; actor: string; detail: string }) {
        return callGateway<{ incident: Incident }>('incidents.addTimeline', params);
      },
      async notifyStakeholders(params: { incidentId: string; channels: string[]; stakeholders: string[] }) {
        return callGateway<{ success: boolean; notifiedAt: string }>('incidents.notifyStakeholders', params);
      },
    },
    ...
    rollbacks: {
      ...
      async initiate(params: { incidentId: string; fromVersion: string; toVersion: string; rollbackType: RollbackType }) {
    ...
    changeRequests: {
      ...
      async get(changeRequestId: string) {
        return callGateway<{ changeRequest: ChangeRequest }>('changeRequests.get', { changeRequestId });
      },
      async create(params: { title: string; description?: string; changeType: string; gitBranch?: string; prUrl?: string }) {
