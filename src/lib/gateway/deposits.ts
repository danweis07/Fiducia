  ...
  return {
    ...
    statements: {
      ...
      async download(id: string) {
        return callGateway<{ downloadUrl: string; expiresAt: string }>(
    ...
    checks: {
      ...
      async createOrder(params: {
        ...
        startingCheckNumber?: string; shippingMethod: 'standard' | 'expedited' | 'overnight';
        deliveryAddressId?: string;
      }) {
        return callGateway<{ order: CheckOrder }>('checks.order.create', params);
      },
      async listOrders(params: { status?: CheckOrderStatus; limit?: number; offset?: number } = {}) {
        return callGateway<{ orders: CheckOrder[]; _pagination?: Pagination }>('checks.orders.list', params);
      },
    ...
    directDeposit: {
      async employers(params: { query?: string; limit?: number; offset?: number } = {}) {
        return callGateway<{ employers: SupportedEmployer[]; _pagination?: Pagination }>('directDeposit.employers', params);
      },
      async initiate(params: {
        ...
        return callGateway<{ switch: DirectDepositSwitch; widgetUrl: string; linkToken: string }>('directDeposit.initiate', params);
      },
      async status(switchId: string) {
        return callGateway<{ switch: DirectDepositSwitch }>('directDeposit.status', { switchId });
      },
      async list(params: { limit?: number; offset?: number } = {}) {
        return callGateway<{ switches: DirectDepositSwitch[]; _pagination?: Pagination }>('directDeposit.list', params);
      },
      ...
      async confirm(switchId: string, providerConfirmationId: string) {
        return callGateway<{ switch: DirectDepositSwitch }>('directDeposit.confirm', { switchId, providerConfirmationId });
      },
    },
  };
}
