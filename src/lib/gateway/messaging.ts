  ...
  return {
    messaging: {
      async listThreads(params: { limit?: number; offset?: number; status?: string } = {}) {
        return callGateway<{ threads: MessageThread[]; _pagination?: Pagination }>('messaging.threads.list', params);
      },
      async getThread(threadId: string) {
        return callGateway<{ thread: MessageThread; messages: Message[] }>('messaging.threads.get', { threadId });
      },
      async createThread(params: { subject: string; body: string; departmentId?: string; priority?: 'normal' | 'urgent' }) {
    ...
    disputes: {
      ...
      async list(params: { status?: string; limit?: number; offset?: number } = {}) {
        return callGateway<{ disputes: Dispute[]; _pagination?: Pagination }>('disputes.list', params);
      },

      async get(disputeId: string) {
        return callGateway<{ dispute: Dispute; timeline: DisputeTimelineEvent[]; documents: DisputeDocument[] }>(
    ...
    vault: {
      async list(params: { limit?: number; offset?: number; category?: VaultDocumentCategory } = {}) {
      ...
      async upload(params: {
        ...
        return callGateway<{ document: VaultDocument; uploadUrl: string | null }>('vault.documents.upload', params);
      },
    ...
    jointAccounts: {
      async listOwners(accountId: string, params: { limit?: number; offset?: number } = {}) {
        return callGateway<{ owners: JointAccountOwner[]; _pagination?: Pagination }>(
          'jointAccounts.owners.list', { accountId, ...params }
        );
      },
      async addOwner(params: {
        ...
        return callGateway<{ invitation: JointAccountInvitation }>(
          'jointAccounts.owners.add', params
        );
      },
      async removeOwner(accountId: string, ownerId: string) {
        return callGateway<{ success: boolean }>(
          'jointAccounts.owners.remove', { accountId, ownerId }
        );
      },
      async updatePermissions(accountId: string, ownerId: string, permissions: JointOwnerPermission) {
        return callGateway<{ owner: JointAccountOwner }>(
          'jointAccounts.owners.updatePermissions', { accountId, ownerId, permissions }
        );
      },
      async listInvitations(params: { limit?: number; offset?: number } = {}) {
        return callGateway<{ invitations: (JointAccountInvitation & { direction: 'sent' | 'received' })[]; _pagination?: Pagination }>(
          'jointAccounts.invitations.list', params
        );
      },
      async acceptInvitation(invitationId: string) {
        return callGateway<{ success: boolean; invitationId: string }>(
          'jointAccounts.invitations.accept', { invitationId }
        );
      },
      async declineInvitation(invitationId: string) {
        return callGateway<{ success: boolean; invitationId: string }>(
          'jointAccounts.invitations.decline', { invitationId }
        );
      },
      async summary() {
        return callGateway<{
