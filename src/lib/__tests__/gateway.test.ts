...
describe('Gateway Client', () => {
  ...
  describe('transactions', () => {
    it('list calls with filter parameters', async () => {
      ...
      await gateway.transactions.list({ accountId: 'a1', category: 'groceries', limit: 10, offset: 0 });
      expect(invoke).toHaveBeenCalledWith('transactions.list', {
        accountId: 'a1', category: 'groceries', limit: 10, offset: 0,
      });
    });
    ...
    it('search passes query and params', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('transactions.search', {
        query: 'coffee', accountId: 'a1', limit: 5,
      });
    });
  });
  ...
  describe('transfers', () => {
    ...
    it('list passes pagination params', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('transfers.list', { status: 'pending', limit: 20, offset: 0 });
    });

    it('schedule passes input with scheduledDate', async () => {
      const input = { fromAccountId: 'a1', toAccountId: 'a2', type: 'internal', amountCents: 1000, scheduledDate: '2026-04-01' };
  ...
  describe('rdc', () => {
    it('deposit passes input fields', async () => {
      const input = { accountId: 'a1', amountCents: 50000, frontImageBase64: 'img1', backImageBase64: 'img2', checkNumber: '1001' };
  ...
  describe('bills', () => {
    it('create passes full input', async () => {
      const input = { payeeName: 'Electric Co', payeeAccountNumber: '123', amountCents: 15000, dueDate: '2026-03-20', fromAccountId: 'a1' };
  ...
  describe('financial', () => {
    it('spending passes period params', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('financial.spending', expect.objectContaining({ periodStart: '2026-03-01' }));
    });
    ...
    it('setBudget passes category and limit', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('financial.budgets.set', { category: 'food', limitCents: 50000 });
    });
  ...
  describe('activation', () => {
    ...
    it('verifyIdentity passes params', async () => {
      ...
      await gateway.activation.verifyIdentity({ memberNumber: '12345', ssn: '1234', dateOfBirth: '1990-01-01' } as Record<string, unknown>);
      expect(invoke).toHaveBeenCalledWith('activation.verifyIdentity', expect.objectContaining({ memberNumber: '12345' }));
    });

    it('acceptTerms passes acceptances', async () => {
      const invoke = mockInvoke({ accepted: true });
      await gateway.activation.acceptTerms({ activationToken: 'tok', acceptances: [{ documentId: 'd1', version: '1' }] });
      expect(invoke).toHaveBeenCalledWith('activation.acceptTerms', expect.objectContaining({ activationToken: 'tok' }));
    });
  ...
  describe('beneficiaries', () => {
    ...
    it('create passes input', async () => {
      const input = { name: 'John', accountNumber: '1234', routingNumber: '5678', bankName: 'Bank', type: 'ach' };
  ...
  describe('enrichment', () => {
    it('enhance passes params', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('enrichment.enhance', expect.objectContaining({ description: 'Coffee' }));
    });
    ...
    it('batch passes transactions', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('enrichment.batch', expect.objectContaining({ transactions: expect.any(Array) }));
    });
  });
  ...
  describe('kyc', () => {
    it('evaluate passes identity params', async () => {
      ...
      await gateway.kyc.evaluate({
        firstName: 'John', lastName: 'Doe', email: 'j@d.com', phone: '5551234',
        dateOfBirth: '1990-01-01', ssn: '123456789',
        address: { line1: '123 Main', city: 'NY', state: 'NY', zip: '10001' },
      });
      expect(invoke).toHaveBeenCalledWith('kyc.evaluate', expect.objectContaining({ firstName: 'John' }));
    });
  ...
  describe('cms', () => {
    ...
    it('createContent passes input', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('cms.content.create', { slug: 'about', title: 'About Us' });
    });
  ...
  describe('auth', () => {
    ...
    it('updateProfile passes updates', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('auth.updateProfile', expect.objectContaining({ firstName: 'Jane' }));
    });
  });
  ...
  describe('locations', () => {
    it('search passes lat/lng', async () => {
      ...
      expect(invoke).toHaveBeenCalledWith('locations.search', expect.objectContaining({ latitude: 37.7749 }));
    });
  });
