...
export class AblyRealtimeProvider implements RealtimeProvider {
  ...
  private async ensureClient(): Promise<AblyClient> {
    ...
    if (!this.initPromise) {
      this.initPromise = (async () => {
        ...
        const Ably = await (new Function('m', 'return import(m)') as (m: string) => Promise<any>)(ablyModuleName);
  ...
  subscribe(config: RealtimeSubscription): Unsubscribe {
    ...
    config.onError?.(new Error('Ably provider does not support table-change subscriptions without a fallback provider'));
    return { unsubscribe: () => {} };
  }
  ...
  trackPresence(channel: string, state: Record<string, unknown>): void {
    ...
    this.getChannel(channel).then((ch) => ch.presence.enter(state)).catch(() => {});
  }
    ...
    const handler = (msg: AblyMessage) => {
      try {
        const message: ChannelMessage = {
          ...
          data: (typeof msg.data === 'object' && msg.data !== null ? msg.data : { value: msg.data }) as Record<string, unknown>,
