...
export class KafkaRealtimeProvider implements RealtimeProvider {
  ...
  private async ensureConnection(): Promise<WebSocket> {
    ...
    if (!this.connectPromise) {
      this.connectPromise = new Promise<void>((resolve, reject) => {
        ...
        const group = this.config.consumerGroup ?? import.meta.env.VITE_KAFKA_CONSUMER_GROUP ?? 'fiducia-web';
  ...
  subscribe(config: RealtimeSubscription): Unsubscribe {
    ...
    config.onError?.(new Error('Kafka provider does not support table-change subscriptions without a fallback provider'));
    return { unsubscribe: () => {} };
  }
    ...
    const handler = (msg: KafkaBridgeMessage) => {
      ...
      try {
        const message: ChannelMessage = {
          id: msg.offset !== undefined ? `${msg.partition ?? 0}:${msg.offset}` : crypto.randomUUID(),
          channel: topic,
          event: eventName,
          data: msg.value ?? {},
