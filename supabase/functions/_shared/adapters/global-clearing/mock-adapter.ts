...
const MOCK_RATES: Record<string, number> = {
  ...
  CAD: 1.36, AUD: 1.53, SGD: 1.34, HKD: 7.82, SEK: 10.45,
  NOK: 10.72, DKK: 6.87, PLN: 3.97, CZK: 23.15, BRL: 4.97,
  INR: 83.12, MXN: 17.15, ZAR: 18.65, CNY: 7.24, KRW: 1325.0,
};
...
export class MockGlobalClearingAdapter implements GlobalClearingAdapter {
  ...
  async createVirtualAccount(request: CreateVirtualAccountRequest): Promise<VirtualAccount> {
    ...
    const iban = `${request.country}00MOCK${Math.random().toString().slice(2, 18)}`;
    const account: VirtualAccount = {
      ...
      availableRails: request.country === 'GB' ? ['faster_payments', 'bacs', 'chaps', 'swift'] : ['sepa', 'sepa_instant', 'swift'],
  ...
  async listPayments(request: ListPaymentsRequest): Promise<ListPaymentsResponse> {
    ...
    if (request.sourceAccountId) payments = payments.filter((p) => p.sourceAccountId === request.sourceAccountId);
