...
export const aggregatorKeys = {
  ...
  institutions: (query: string, countryCode?: string) =>
    ["aggregator", "institutions", query, countryCode] as const,
...
export function useRefreshAggregatorConnection() {
  ...
  return useMutation({
    mutationFn: (connectionId: string) => gateway.aggregator.refreshConnection(connectionId),
    onSuccess: () => {
...
export function useRemoveAggregatorConnection() {
  ...
  return useMutation({
    mutationFn: (connectionId: string) => gateway.aggregator.removeConnection(connectionId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: aggregatorKeys.all });
    },
  });
