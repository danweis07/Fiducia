...
describe("billKeys", () => {
  ...
  it("has correct list key", () => {
    expect(billKeys.list({ status: "scheduled" })).toEqual([
      "bills",
      "list",
      { status: "scheduled" },
    ]);
...
describe("useBills", () => {
  ...
  it("passes status filter", async () => {
    ...
    const { result } = renderHook(() => useBills({ status: "scheduled" }), {
  ...
  it("passes pagination", async () => {
    ...
    const { result } = renderHook(() => useBills({ limit: 10, offset: 20 }), {
      wrapper: createWrapper(),
    });
...
describe("useCreateBill", () => {
  ...
  it("creates a bill with autopay", async () => {
    ...
    expect(gateway.bills.create).toHaveBeenCalledWith(expect.objectContaining({ autopay: true }));
  });
...
describe("usePayBill", () => {
  ...
  it("pays a bill", async () => {
    vi.mocked(gateway.bills.pay).mockResolvedValue({
      bill: { id: "bill-1", status: "paid" },
    } as never);
