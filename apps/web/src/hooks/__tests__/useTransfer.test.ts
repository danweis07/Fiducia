...
describe("transferKeys", () => {
  ...
  it("has correct list key with params", () => {
    expect(transferKeys.list({ status: "pending" })).toEqual([
      "transfers",
      "list",
      { status: "pending" },
    ]);
...
describe("useTransfers", () => {
  ...
  it("passes status filter", async () => {
    ...
    const { result } = renderHook(() => useTransfers({ status: "pending" }), {
  ...
  it("passes pagination params", async () => {
    ...
    const { result } = renderHook(() => useTransfers({ limit: 10, offset: 5 }), {
      wrapper: createWrapper(),
    });
