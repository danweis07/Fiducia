...
export function loanPayment(principalCents: number, rateBps: number, termMonths: number): number {
  const r = rateBps / 10000 / 12;
  if (r === 0) return Math.round(principalCents / termMonths);
  const payment =
    (principalCents * (r * Math.pow(1 + r, termMonths))) / (Math.pow(1 + r, termMonths) - 1);
...
export function savingsGoal(goalCents: number, rateBps: number, months: number): number {
  const r = rateBps / 10000 / 12;
