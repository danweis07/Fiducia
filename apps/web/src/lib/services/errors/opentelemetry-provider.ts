...
export class OpenTelemetryProvider implements ErrorTrackingProvider {
  ...
  init(config: Record<string, unknown>): void {
    ...
    try {
      ...
      console.debug(
        `[OTel] Initialized (service=${serviceName}, env=${environment}, endpoint=${endpoint})`,
      );
    } catch (error) {
      console.warn(
        "[OTel] OpenTelemetry packages not installed. Run: npm install @opentelemetry/api @opentelemetry/sdk-trace-web @opentelemetry/exporter-trace-otlp-http @opentelemetry/resources @opentelemetry/semantic-conventions",
      );
  ...
  private flattenAttributes(prefix: string, obj?: Record<string, unknown>): Record<string, string> {
    if (!obj) return {};
