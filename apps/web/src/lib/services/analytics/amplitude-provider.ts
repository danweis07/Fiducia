...
export class AmplitudeProvider implements AnalyticsProvider {
  ...
  async init(config: Record<string, unknown>): Promise<void> {
    ...
    try {
      ...
      console.warn(
        "[Amplitude] @amplitude/analytics-browser not installed. Run: npm install @amplitude/analytics-browser",
      );
    }
  }
  ...
  revenue(amount: number, properties?: Record<string, unknown>): void {
    if (!this.amp) return;
    const revenueObj = new this.amp.Revenue().setPrice(amount).setQuantity(1);
