  ...
  return {
    ...
    billpay: {
      ...
      async schedulePayment(input: {
        ...
        method?: string;
        memo?: string;
        recurringRule?: { frequency: string; dayOfMonth?: number; endDate?: string };
      }) {
        return callGateway<{ payment: Record<string, unknown> }>(
          "billpay.payments.schedule",
          input,
        );
      },

      async cancelPayment(paymentId: string) {
        return callGateway<{ success: boolean; payment: Record<string, unknown> }>(
      ...
      async listPayments(
        ...
        return callGateway<{ payments: Array<Record<string, unknown>>; _pagination?: Pagination }>(
    ...
    wires: {
      ...
      async list(
        ...
        return callGateway<{ wires: WireTransfer[]; _pagination?: Pagination }>(
          "wires.list",
          params,
        );
      },
    ...
    stopPayments: {
      ...
      async list(
        ...
        return callGateway<{ stopPayments: StopPayment[]; _pagination?: Pagination }>(
          "stopPayments.list",
          params,
        );
      },
      ...
      async renew(stopPaymentId: string, duration: "6months" | "12months" | "permanent") {
        return callGateway<{ stopPayment: StopPayment }>("stopPayments.renew", {
          stopPaymentId,
          duration,
        });
      },
    ...
    p2p: {
      ...
      async listTransactions(
        params: { filter?: "sent" | "received" | "requests"; limit?: number; offset?: number } = {},
      ) {
        return callGateway<{ transactions: P2PTransaction[]; _pagination?: Pagination }>(
          "p2p.transactions",
          params,
        );
      },
    ...
    instantPayments: {
      ...
      async checkReceiver(params: {
        ...
        }>("instantPayments.checkReceiver", params);
      },
      async requestForPayment(params: {
        ...
        }>("instantPayments.requestForPayment", params);
      },
      async exportISO20022(paymentId: string, format?: "pain.001" | "pacs.008") {
        return callGateway<{ xml: string; messageType: string; paymentId: string }>(
          "instantPayments.exportISO20022",
          { paymentId, format },
        );
      },
      async getLimits() {
        return callGateway<{
          limits: import("@/types").InstantPaymentLimits;
          supportedCurrencies: string[];
          supportedRails: string[];
        }>("instantPayments.limits", {});
      },
    },
  };
}
