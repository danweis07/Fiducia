    ...
    aiPlatform: {
      ...
      automation: {
        ...
        async create(params: { description: string }) {
          ...
          }>("ai.automation.create", params);
        },
        async update(ruleId: string, updates: { status?: string; name?: string }) {
          return callGateway<{ rule: { id: string; status: string } }>("ai.automation.update", {
            ruleId,
            ...updates,
          });
        },
        ...
        async history(ruleId: string, params: { limit?: number } = {}) {
          ...
          }>("ai.automation.history", { ruleId, ...params });
        },
      },
      ...
      insights: {
        async list(params: { status?: string; type?: string } = {}) {
          ...
          }>("ai.insights.list", params);
        },
        async act(insightId: string) {
          return callGateway<{ success: boolean; actionResult?: Record<string, unknown> }>(
            "ai.insights.act",
            { insightId },
          );
        },
        ...
        async generate() {
          return callGateway<{
            insights: Array<{ type: string; title: string; message: string; severity: string }>;
      ...
      escalations: {
        ...
        async get(escalationId: string) {
          ...
          }>("ai.escalations.get", { escalationId });
        },
        async assign(escalationId: string, assignedTo: string) {
          return callGateway<{ success: boolean }>("ai.escalations.assign", {
            escalationId,
            assignedTo,
          });
        },
        async resolve(escalationId: string, resolutionNotes: string) {
          return callGateway<{ success: boolean }>("ai.escalations.resolve", {
            escalationId,
            resolutionNotes,
          });
        },
      ...
      prompts: {
        ...
        async update(
          ...
          return callGateway<{ prompt: { id: string; version: number } }>(
            "ai.platform.prompts.update",
            { promptId, ...updates },
          );
        },
        async test(params: { stakeholder: string; message: string; customContent?: string }) {
          return callGateway<{ reply: string; tokensUsed: number }>(
