...
export function createFraudGraphDomain(callGateway: CallGatewayFn) {
  return {
    fraudGraph: {
      ...
      async getCustomerGraph(params: { customerId: string; depth?: number }) {
        return callGateway<{ graph: CustomerGraph }>("fraud.graph.network", params);
      },

      async detectCommunities(params?: { minCommunitySize?: number; riskThreshold?: number }) {
        return callGateway<{
          communities: Community[];
          totalCommunities: number;
          analyzedAt: string;
        }>("fraud.graph.communities", params ?? {});
      },

      async propagateRisk(params: { customerId: string; propagationDepth?: number }) {
