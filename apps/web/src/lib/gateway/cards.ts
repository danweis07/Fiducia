  ...
  return {
    ...
    cardProvisioning: {
      ...
      async checkEligibility(cardId: string, walletProvider: WalletProvider) {
        return callGateway<{ eligibility: ProvisioningEligibility }>(
          "cardProvisioning.checkEligibility",
          { cardId, walletProvider },
        );
      },

      async initiate(cardId: string, walletProvider: WalletProvider, deviceId?: string) {
        return callGateway<ProvisioningResult>("cardProvisioning.initiate", {
          cardId,
          walletProvider,
          deviceId,
        });
      ...
      async complete(
        ...
        return callGateway<ProvisioningCompletion>("cardProvisioning.complete", {
          provisioningId,
          cardId,
          walletProvider,
          walletToken,
      ...
      async credentials(cardId: string) {
        return callGateway<{ credentials: CardCredentials }>("cardProvisioning.credentials", {
          cardId,
        });
      },
      ...
      async setTempExpiration(cardId: string) {
        return callGateway<{
          ...
          setOnSwitch: boolean;
          setOnCore: boolean;
        }>("cardProvisioning.setTempExpiration", { cardId });
      },

      async requestDigitalOnly(accountId: string) {
        return callGateway<{ card: DigitalOnlyCard }>("cardProvisioning.digitalOnly", {
          accountId,
        });
      },
      ...
      async requestPhysical(cardId: string) {
        ...
        }>("cardProvisioning.requestPhysical", { cardId });
      },

      async reportAndReplace(cardId: string, reason: "lost" | "stolen", digitalOnly?: boolean) {
        return callGateway<CardReplacementResult>("cardProvisioning.reportReplace", {
          cardId,
          reason,
          digitalOnly,
        });
    ...
    cardReplacements: {
      async request(params: {
        ...
        return callGateway<{ replacement: CardReplacement }>(
          "cardServices.replacement.request",
          params,
        );
      },
      async list(params: { limit?: number; offset?: number } = {}) {
        return callGateway<{ replacements: CardReplacement[] }>(
          "cardServices.replacement.list",
          params,
        );
      },
      async status(replacementId: string) {
        return callGateway<{ replacement: CardReplacement }>("cardServices.replacement.status", {
          replacementId,
        });
      },
      async activate(replacementId: string, lastFourDigits: string) {
        return callGateway<{ success: boolean; replacementId: string }>(
          "cardServices.replacement.activate",
          { replacementId, lastFourDigits },
        );
      },
    },
  };
