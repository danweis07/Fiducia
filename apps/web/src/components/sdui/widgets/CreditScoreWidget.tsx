    ...
    <Card>
      ...
      <CardContent>
        <div className="flex items-baseline gap-3">
          <span className={`text-4xl font-bold ${getScoreColor(scoreNum)}`}>
            {scoreNum || "--"}
          </span>
          {change && (
            <span
              className={`text-sm font-medium ${(change as number) >= 0 ? "text-emerald-600" : "text-red-600"}`}
            >
              {(change as number) >= 0 ? "+" : ""}
              {change as number} pts
            </span>
          )}
        </div>
        {provider && (
          <p className="text-xs text-slate-400 mt-2">Provided by {provider as string}</p>
        )}
      </CardContent>
    </Card>
  );
