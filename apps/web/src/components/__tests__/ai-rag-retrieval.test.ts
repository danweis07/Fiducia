...
describe("chunkDocument", () => {
  ...
  it("splits long document into multiple chunks", () => {
    ...
    const paragraph =
      "This is a paragraph about banking services and account management. It contains multiple sentences that describe various features. Members can check balances, view transactions, and manage their accounts online.";
  ...
  it("chunks have overlap content from previous chunk", () => {
    ...
    for (let i = 0; i < 20; i++) {
      paragraphs.push(
        `Paragraph number ${i}. This paragraph discusses topic ${i} in detail with enough content to matter for chunking.`,
      );
    }
...
describe("formatRAGContext", () => {
  ...
  it("returns empty string when hasResults is false even with chunks", () => {
    const context: RAGContext = {
      chunks: [{ content: "Some content", documentId: "doc-1", similarity: 0.9, metadata: {} }],
