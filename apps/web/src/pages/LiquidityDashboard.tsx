  ...
  const handleCreateVault = () => {
    ...
    createVault.mutate(
      ...
      },
      {
        onSuccess: () => {
          toast({
            title: t("liquidityDashboard.vaultCreated"),
            description: t("liquidityDashboard.vaultCreatedDesc", { name: vaultName }),
          });
    ...
    <AppShell>
      <div className="container mx-auto max-w-6xl py-6 px-4 space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">{t("liquidityDashboard.subtitle")}</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t("liquidityDashboard.currentBalance")}
                  </p>
                  <p className="text-2xl font-bold">
                    {formatCurrency(forecast?.currentBalanceCents ?? 0)}
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t("liquidityDashboard.projected30d")}
                  </p>
                  <p className="text-2xl font-bold">
                    {formatCurrency(forecast?.projectedBalanceCents ?? 0)}
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t("liquidityDashboard.avgDailyInflow")}
                  </p>
                  <p className="text-2xl font-bold">
                    {formatCurrency(forecast?.avgDailyInflowCents ?? 0)}
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card className={(forecast?.runwayDays ?? 999) < 60 ? "border-yellow-300" : ""}>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${(forecast?.runwayDays ?? 999) < 60 ? "bg-yellow-100" : "bg-gray-100"}`}
                >
                  <TrendingDown
                    className={`w-5 h-5 ${(forecast?.runwayDays ?? 999) < 60 ? "text-yellow-600" : "text-gray-600"}`}
                  />
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t("liquidityDashboard.cashRunway")}
                  </p>
                  <p className="text-2xl font-bold">
                    {(forecast?.runwayDays ?? 0) >= 999
                      ? t("liquidityDashboard.infinite")
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t("liquidityDashboard.cashFlowForecastDesc", {
                date: forecast?.projectedDate ?? "--",
                amount: formatCurrency(forecast?.projectedBalanceCents ?? 0),
              })}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {forecastLoading ? (
              <div className="text-center py-12 text-muted-foreground">
                {t("liquidityDashboard.analyzingHistory")}
              </div>
            ) : (
              <>
                    ...
                    .map((point, idx) => {
                      ...
                      return (
                        <div
                          ...
                          title={`${point.date}: ${formatCurrency(point.balanceCents)}`}
                        >
                          <div
          ...
          <div className="flex items-center justify-between">
            <TabsList>
              ...
              <TabsTrigger value="upcoming">
                {t("liquidityDashboard.upcomingObligations")}
              </TabsTrigger>
            </TabsList>
            <Button
              size="sm"
                  ...
                  {treasurySummary && (
                    <div className="text-right">
                      <p className="text-2xl font-bold text-green-600">
                        {formatBps(treasurySummary.weightedAvgApyBps)} {t("liquidityDashboard.apy")}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        {t("liquidityDashboard.deposited", {
                          amount: formatCurrency(treasurySummary.totalVaultBalanceCents),
                        })}
                      </p>
                    </div>
                  )}
                      ...
                      {vaults.map((vault: TreasuryVault) => (
                        <TableRow key={vault.id}>
                          ...
                          <TableCell className="font-semibold">
                            {formatCurrency(vault.balanceCents)}
                          </TableCell>
                          <TableCell className="text-green-600 font-medium">
                            {formatBps(vault.apyBps)}
                          </TableCell>
                          <TableCell className="text-green-600">
                            {formatCurrency(vault.accruedInterestCents)}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-right">
                            {vault.status === "active" && (
                              <Button
                                ...
                                variant="ghost"
                                onClick={() =>
                                  closeVault.mutate(vault.id, {
                                    onSuccess: () =>
                                      toast({
                                        title: t("liquidityDashboard.vaultClosed"),
                                        description: t("liquidityDashboard.vaultClosedDesc"),
                                      }),
                                  })
                                }
                              >
          ...
          <TabsContent value="upcoming">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {t("liquidityDashboard.upcomingCashObligationsDesc")}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-4 rounded-lg border bg-muted/20">
                    <p className="text-sm text-muted-foreground">
                      {t("liquidityDashboard.estPayroll")}
                    </p>
                    <p className="text-xl font-bold mt-1">
                      {formatCurrency(forecast?.upcomingPayrollCents ?? 0)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {t("liquidityDashboard.estPayrollBasis")}
                    </p>
                  </div>
                  <div className="p-4 rounded-lg border bg-muted/20">
                    <p className="text-sm text-muted-foreground">
                      {t("liquidityDashboard.estRecurringBills")}
                    </p>
                    <p className="text-xl font-bold mt-1">
                      {formatCurrency(forecast?.upcomingBillsCents ?? 0)}
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      {t("liquidityDashboard.estRecurringBillsBasis")}
                    </p>
                  </div>
                  <div className="p-4 rounded-lg border bg-muted/20">
                    <p className="text-sm text-muted-foreground">
                      {t("liquidityDashboard.avgDailyOutflow")}
                    </p>
                    <p className="text-xl font-bold mt-1">
                      {formatCurrency(forecast?.avgDailyOutflowCents ?? 0)}
                    </p>
                  </div>
                  <div className="p-4 rounded-lg border bg-muted/20">
                    <p className="text-sm text-muted-foreground">
                      {t("liquidityDashboard.netDailyChange")}
                    </p>
                    <p className="text-xl font-bold mt-1">
                      {formatCurrency(
                        (forecast?.avgDailyInflowCents ?? 0) -
                          (forecast?.avgDailyOutflowCents ?? 0),
                      )}
                    </p>
                  </div>
                </div>
        ...
        <Dialog open={vaultDialogOpen} onOpenChange={setVaultDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>{t("liquidityDashboard.openTreasuryVaultDesc")}</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                <Label>{t("liquidityDashboard.vaultName")}</Label>
                <Input
                  placeholder={t("liquidityDashboard.vaultNamePlaceholder")}
                  value={vaultName}
              ...
              <div>
                ...
                <Select value={vaultAccountId} onValueChange={setVaultAccountId}>
                  <SelectTrigger>
                    <SelectValue placeholder={t("liquidityDashboard.selectAccount")} />
                  </SelectTrigger>
                  <SelectContent>
                    {accounts.map((acc) => (
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setVaultDialogOpen(false)}>
                {t("liquidityDashboard.cancel")}
              </Button>
              <Button onClick={handleCreateVault} disabled={createVault.isPending}>
                {createVault.isPending
                  ? t("liquidityDashboard.creating")
                  : t("liquidityDashboard.openVault")}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
