      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="travel">
            ...
            {t("cardServices.travelNotices")}
          </TabsTrigger>
          <TabsTrigger value="replacement">
            <CreditCard className="h-4 w-4 mr-2" />
            {t("cardServices.cardReplacement")}
          </TabsTrigger>
        </TabsList>
...
function TravelNoticesTab({ toast, handleError }: TabProps) {
  ...
  const handleCreate = async () => {
    ...
    if (!cardId || validDests.length === 0 || !startDate || !endDate) {
      toast({
        title: t("cardServices.missingFields"),
        description: t("cardServices.fillRequiredFields"),
        variant: "destructive",
      });
      return;
    }
    try {
      ...
      toast({
        title: t("cardServices.noticeCreated"),
        description: t("cardServices.noticeCreatedDesc"),
      });
      setShowForm(false);
      setCardId("");
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center text-destructive">
          {t("cardServices.failedLoadNotices")}
        </CardContent>
      </Card>
    );
  ...
  return (
    <>
      ...
      {notices.length === 0 ? (
        <EmptyState
          icon={Plane}
          title={t("cardServices.noTravelNotices")}
          description={t("cardServices.noTravelNoticesDesc")}
        />
      ) : (
                  ...
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={n.isActive ? "default" : "secondary"}>
                        {n.isActive ? t("cardServices.active") : t("cardServices.expired")}
                      </Badge>
                    </div>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select value={cardId} onValueChange={setCardId}>
                <SelectTrigger>
                  <SelectValue placeholder={t("cardServices.selectCard")} />
                </SelectTrigger>
                <SelectContent>
                  {cards.map((c) => (
            ...
            <div>
              ...
              {destinations.map((d, i) => (
                <div key={i} className="flex gap-2 mt-2">
                  <Input
                    placeholder={t("cardServices.country")}
                    value={d.country}
                    onChange={(e) => updateDestination(i, "country", e.target.value)}
                  />
                  <Input
                    placeholder={t("cardServices.regionOptional")}
                    value={d.region}
                    onChange={(e) => updateDestination(i, "region", e.target.value)}
                  />
              ...
              <Button variant="link" size="sm" className="mt-1 p-0" onClick={addDestination}>
                <Plus className="h-3 w-3 mr-1" />
                {t("cardServices.addDestination")}
              </Button>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>{t("cardServices.startDate")}</Label>
                <Input
                  type="date"
                  value={startDate}
              ...
              <div>
                <Label>{t("cardServices.endDate")}</Label>
                <Input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
              </div>
            </div>
            <div>
              <Label>{t("cardServices.contactPhoneOptional")}</Label>
              <Input
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowForm(false)}>
              {t("cardServices.cancel")}
            </Button>
            <Button onClick={handleCreate} disabled={createNotice.isPending}>
              {createNotice.isPending ? t("cardServices.creating") : t("cardServices.createNotice")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("cardServices.cancelTravelNoticeDesc")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function CardReplacementTab({ toast, handleError }: TabProps) {
  ...
  const handleRequest = async () => {
    if (!cardId || !reason) {
      toast({
        title: t("cardServices.missingFields"),
        description: t("cardServices.selectCardAndReason"),
        variant: "destructive",
      });
      return;
    }
    try {
      ...
      toast({
        title: t("cardServices.replacementRequested"),
        description: t("cardServices.replacementRequestedDesc"),
      });
      setShowForm(false);
      setCardId("");
      setReason("");
  ...
  const handleActivate = async () => {
    if (!activateDialog || !lastFour || lastFour.length !== 4) {
      toast({
        title: t("cardServices.invalidInput"),
        description: t("cardServices.enterLastFour"),
        variant: "destructive",
      });
      return;
    }
    try {
      ...
      toast({
        title: t("cardServices.cardActivated"),
        description: t("cardServices.cardActivatedDesc"),
      });
  ...
  if (error)
    return (
      <Card>
        <CardContent className="py-8 text-center text-destructive">
          {t("cardServices.failedLoadReplacements")}
        </CardContent>
      </Card>
    );
  ...
  return (
    <>
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => setShowForm(true)}>
          <Plus className="h-4 w-4 mr-2" />
          {t("cardServices.requestReplacement")}
        </Button>
      </div>
      ...
      {replacements.length === 0 ? (
        <EmptyState
          icon={Package}
          title={t("cardServices.noReplacements")}
          description={t("cardServices.noReplacementsDesc")}
        />
      ) : (
        <div className="grid gap-4">
          {replacements.map((r) => (
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      {r.fraudReported && (
                        <Badge variant="destructive">{t("cardServices.fraudReported")}</Badge>
                      )}
                    </div>
                    <div className="flex items-center gap-4 text-sm text-muted-foreground">
                      <span>
                        {t("cardServices.reason")}: {reasonLabels[r.reason]}
                      </span>
                      <span className="flex items-center gap-1">
                      ...
                      {r.estimatedDeliveryDate && (
                        <span>
                          {t("cardServices.estDelivery")}: {r.estimatedDeliveryDate}
                        </span>
                      )}
                      {r.feeCents > 0 && (
                        <span>
                          {t("cardServices.fee")}: {formatCurrency(r.feeCents)}
                        </span>
                      )}
                    </div>
                    {r.trackingNumber && (
                      <p className="text-xs text-muted-foreground">
                        {t("cardServices.tracking")}: {r.trackingNumber}
                      </p>
                    )}
                  </div>
                  {(r.status === "shipped" || r.status === "delivered") && (
                    <Button size="sm" onClick={() => setActivateDialog(r.id)}>
                      <CheckCircle className="h-4 w-4 mr-1" />
                      {t("cardServices.activate")}
                    </Button>
                  )}
                </div>
              </CardContent>
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select value={reason} onValueChange={(v) => setReason(v as CardReplacementReason)}>
                <SelectTrigger>
                  <SelectValue placeholder={t("cardServices.selectReason")} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setShippingMethod(v as "standard" | "expedited")}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleRequest} disabled={requestReplacement.isPending}>
              {requestReplacement.isPending
                ? t("cardServices.requesting")
                : t("cardServices.requestReplacement")}
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      >
        <DialogContent>
          ...
          <div>
            <Label>{t("cardServices.lastFourDigits")}</Label>
            <Input
              maxLength={4}
              placeholder="1234"
              value={lastFour}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleActivate} disabled={activateCard.isPending}>
              {activateCard.isPending
                ? t("cardServices.activating")
                : t("cardServices.activateCard")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
