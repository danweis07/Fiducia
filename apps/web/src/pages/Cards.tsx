...
import { CreditCard, Lock, Unlock, Wifi, Eye, EyeOff, ShieldCheck } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function Cards() {
  ...
  const toggleLock = async (cardId: string, currentStatus: string) => {
    try {
      if (currentStatus === "active") {
        ...
        toast({
          title: t("cards.cardUnlockedToast"),
          description: t("cards.cardUnlockedToastDesc"),
        });
      }
    } catch (err) {
  ...
  const handleLimitCommit = async (cardId: string) => {
    ...
    try {
      ...
      toast({
        title: t("cards.limitUpdated"),
        description: t("cards.limitUpdatedDesc", { amount: formatCurrency(limitCents) }),
      });
    } catch (err) {
      handleError(err, { fallbackTitle: t("cards.actionFailed") });
    }
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 space-y-8">
      <div>
        ...
        <p className="text-muted-foreground">{t("cards.pageSubtitle")}</p>
      </div>
      ...
      <AlertDialog
        open={!!confirmLockCardId}
        onOpenChange={(open) => {
          if (!open) setConfirmLockCardId(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("cards.lockThisCard")}</AlertDialogTitle>
            <AlertDialogDescription>{t("cards.lockThisCardDesc")}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
