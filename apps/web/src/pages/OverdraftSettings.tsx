...
export default function OverdraftSettingsPage() {
  ...
  const { data: settingsData, isLoading: loadingSettings } = useOverdraftSettings(
  ...
  const applySettings = () => {
    if (settings) {
      ...
      setCourtesyPayLimit(
        settings.courtesyPayLimitCents ? (settings.courtesyPayLimitCents / 100).toString() : "",
      );
    ...
    updateSettings.mutate(
      {
        ...
        courtesyPayLimitCents:
          protectionType === "courtesy_pay" && courtesyPayLimit
            ? Math.round(parseFloat(courtesyPayLimit) * 100)
            : null,
      ...
      },
      {
        onSuccess: () =>
          toast({
            title: t("overdraftSettings.settingsSaved"),
            description: t("overdraftSettings.settingsSavedDesc"),
          }),
        onError: () =>
          toast({
            title: t("overdraftSettings.error"),
            description: t("overdraftSettings.failedToSave"),
            variant: "destructive",
          }),
      },
    );
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">
                  {t("overdraftSettings.protectionSettings")}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <Label htmlFor="overdraft-toggle" className="text-base font-medium">
                      {t("overdraftSettings.enableProtection")}
                    </Label>
                    <p className="text-sm text-muted-foreground">
                      {t("overdraftSettings.enableProtectionDesc")}
                    </p>
                  </div>
                  <Switch
                    id="overdraft-toggle"
                  ...
                  <>
                    ...
                    <div className="space-y-4">
                      <Label className="text-base font-medium">
                        {t("overdraftSettings.protectionType")}
                      </Label>
                      <RadioGroup
                        value={protectionType}
                        onValueChange={(v) => setProtectionType(v as OverdraftProtectionType)}
                      >
                        <div className="flex items-start gap-3 p-3 rounded-lg border">
                          <RadioGroupItem value="transfer" id="type-transfer" className="mt-0.5" />
                          <div>
                            <Label htmlFor="type-transfer" className="font-medium">
                              {t("overdraftSettings.linkedAccountTransfer")}
                            </Label>
                            <p className="text-sm text-muted-foreground">
                              {t("overdraftSettings.linkedAccountTransferDesc")}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3 p-3 rounded-lg border">
                          ...
                          <div>
                            <Label htmlFor="type-loc" className="font-medium">
                              {t("overdraftSettings.lineOfCredit")}
                            </Label>
                            <p className="text-sm text-muted-foreground">
                              {t("overdraftSettings.lineOfCreditDesc")}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3 p-3 rounded-lg border">
                          ...
                          />
                          <div>
                            <Label htmlFor="type-courtesy" className="font-medium">
                              {t("overdraftSettings.courtesyPay")}
                            </Label>
                            <p className="text-sm text-muted-foreground">
                              {t("overdraftSettings.courtesyPayDesc")}
                            </p>
                          </div>
                        </div>
                      </RadioGroup>
                    ...
                    {protectionType === "courtesy_pay" && (
                      <div className="space-y-2">
                        <Label htmlFor="courtesy-limit">
                          {t("overdraftSettings.courtesyPayLimit")}
                        </Label>
                        <Input
                          id="courtesy-limit"
                          type="number"
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <Label htmlFor="fee-optin" className="text-base font-medium">
                          {t("overdraftSettings.optIntoFees")}
                        </Label>
                        <p className="text-sm text-muted-foreground">
                          {t("overdraftSettings.optIntoFeesDesc")}
                        </p>
                      </div>
                      <Switch
                        id="fee-optin"
                        checked={optedIntoFees}
                        onCheckedChange={setOptedIntoFees}
                      />
                    </div>
                  </>
                )}
                ...
                <div className="pt-4">
                  <Button onClick={handleSave} disabled={updateSettings.isPending}>
                    {updateSettings.isPending
                      ? t("overdraftSettings.saving")
                      : t("overdraftSettings.saveSettings")}
                  </Button>
                </div>
              </CardContent>
            </Card>
                        ...
                        (event: {
                          ...
                          <TableRow key={event.id}>
                            ...
                            <TableCell className="capitalize">
                              {event.protectionType.replace(/_/g, " ")}
                            </TableCell>
                            <TableCell>
                              <Badge variant={event.wasProtected ? "default" : "destructive"}>
                                {event.wasProtected
                                  ? t("overdraftSettings.protectedStatus")
                                  : t("overdraftSettings.notProtected")}
                              </Badge>
                            </TableCell>
                          </TableRow>
                        ),
