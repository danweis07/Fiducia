...
> = {
  pending: {
    ...
    labelKey: "jitPermissions.statusPending",
    variant: "secondary",
  },
  approved: {
    icon: CheckCircle2,
    color: "text-green-500",
    labelKey: "jitPermissions.statusApproved",
    variant: "default",
  },
  denied: {
    icon: XCircle,
    color: "text-red-500",
    labelKey: "jitPermissions.statusDenied",
    variant: "destructive",
  },
  expired: {
    icon: AlertTriangle,
    color: "text-gray-400",
    labelKey: "jitPermissions.statusExpired",
    variant: "outline",
  },
  cancelled: {
    icon: XCircle,
    color: "text-gray-400",
    labelKey: "jitPermissions.statusCancelled",
    variant: "outline",
  },
};
      ...
      { requestId: req.id },
      {
        onSuccess: () => {
          toast({
            ...
            description: t("jitPermissions.toastApprovedDesc", {
              name: req.requesterName,
              action: t(ACTION_LABEL_KEYS[req.actionType]) ?? req.actionType,
            }),
          });
        },
      },
  ...
  const handleDeny = () => {
    ...
    denyRequest.mutate(
      { requestId: denyDialogReq.id, reason: denyReason || undefined },
      {
        onSuccess: () => {
          toast({
            title: t("jitPermissions.toastDenied"),
            description: t("jitPermissions.toastDeniedDesc", { name: denyDialogReq.requesterName }),
          });
    ...
    <AppShell>
      <div className="container mx-auto max-w-6xl py-6 px-4 space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">{t("jitPermissions.subtitle")}</p>
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t("jitPermissions.statusPending")}
                  </p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">
                    {t("jitPermissions.approvedToday")}
                  </p>
        ...
        {pendingRequests.length > 0 && (
          <div className="space-y-3">
            ...
            {pendingRequests.map((req) => (
              <Card
                key={req.id}
                className="border-yellow-200 bg-gradient-to-r from-yellow-50/50 to-transparent"
              >
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-sm text-muted-foreground">
                        ...
                        {req.currentLimitCents !== null && req.requestedLimitCents !== null && (
                          <span>
                            {t("jitPermissions.limit")}: {formatCurrency(req.currentLimitCents)} →{" "}
                            {formatCurrency(req.requestedLimitCents)}
                          </span>
                        )}
                        <span>{timeAgo(req.createdAt)}</span>
                        <Badge variant="outline">
                          {t(ACTION_LABEL_KEYS[req.actionType]) ?? req.actionType}
                        </Badge>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      >
                        ...
                        {t("jitPermissions.approve")}
                        {req.amountCents !== null ? ` ${formatCurrency(req.amountCents)}` : ""}
                      </Button>
                      <Button
                        variant="destructive"
                        size="sm"
                        onClick={() => {
                          setDenyDialogReq(req);
                          setDenyReason("");
                        }}
                      >
                    ...
                    <TableBody>
                      {policies.map((policy: ApprovalPolicy) => (
                        <TableRow key={policy.id}>
                          ...
                          <TableCell>
                            {t(ACTION_LABEL_KEYS[policy.actionType]) ?? policy.actionType}
                          </TableCell>
                            ...
                            <Button
                              ...
                              onClick={() =>
                                deletePolicy.mutate(policy.id, {
                                  onSuccess: () =>
                                    toast({ title: t("jitPermissions.policyDeleted") }),
                                })
                              }
                            >
        ...
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {t("jitPermissions.denyRequestDesc", {
                  name: denyDialogReq?.requesterName,
                  action: denyDialogReq
                    ? (t(ACTION_LABEL_KEYS[denyDialogReq.actionType]) ?? denyDialogReq.actionType)
                    : "",
                })}
              </DialogDescription>
            </DialogHeader>
            <div>
              ...
              <Textarea
                placeholder={t("jitPermissions.denyReasonPlaceholder")}
                value={denyReason}
                onChange={(e) => setDenyReason(e.target.value)}
              />
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setDenyDialogReq(null)}>
                {t("jitPermissions.cancel")}
              </Button>
              <Button variant="destructive" onClick={handleDeny} disabled={denyRequest.isPending}>
                {denyRequest.isPending
                  ? t("jitPermissions.denying")
                  : t("jitPermissions.denyRequest")}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={policyDialogOpen} onOpenChange={setPolicyDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>{t("jitPermissions.createApprovalPolicyDesc")}</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div>
                <Label>{t("jitPermissions.policyName")}</Label>
                <Input
                  placeholder={t("jitPermissions.policyNamePlaceholder")}
              ...
              <div>
                ...
                <Select value={formActionType} onValueChange={setFormActionType}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="card_limit_increase">
                      {t("jitPermissions.actionCardLimitIncrease")}
                    </SelectItem>
                    ...
                    <SelectItem value="account_access">
                      {t("jitPermissions.actionAccountAccess")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <Input
                    type="number"
                    value={formExpireMinutes}
                    onChange={(e) => setFormExpireMinutes(e.target.value)}
                  />
                </div>
              </div>
            </div>
            <DialogFooter>
              ...
              <Button onClick={handleCreatePolicy} disabled={createPolicy.isPending}>
                {createPolicy.isPending
                  ? t("jitPermissions.creating")
                  : t("jitPermissions.createPolicy")}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
