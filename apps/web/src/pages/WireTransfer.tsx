...
export default function WireTransfer() {
  ...
  const feeCents =
    tab === "domestic" ? (fees?.domesticFeeCents ?? 2500) : (fees?.internationalFeeCents ?? 4500);
  ...
  const handleSubmit = async () => {
    try {
      ...
      toast({
        title: t("wireTransfer.toast.initiatedTitle"),
        description: t("wireTransfer.toast.initiatedDescription"),
      });
      resetForm();
    } catch (err) {
  ...
  const handleCancel = async (id: string) => {
    try {
      ...
      toast({
        title: t("wireTransfer.toast.cancelledTitle"),
        description: t("wireTransfer.toast.cancelledDescription"),
      });
    } catch (err) {
  ...
  const isIntlValid =
    fromAccountId &&
    beneficiaryName &&
    /^[A-Za-z]{6}[A-Za-z0-9]{2}([A-Za-z0-9]{3})?$/.test(swiftCode) &&
    iban &&
    bankName &&
    bankCountry &&
    amountCents > 0 &&
  ...
  if (accountsLoading || wiresLoading)
    return (
      <AppShell>
        <PageSkeleton />
      </AppShell>
        ...
        <div className="flex items-center justify-between">
          ...
          {limits && (
            <div className="text-right text-sm text-muted-foreground">
              <p>
                {t("wireTransfer.dailyRemaining")}:{" "}
                <span className="font-semibold text-foreground">
                  {formatCurrency(limits.remainingDailyCents)}
                </span>
              </p>
              <p>
                {t("wireTransfer.perTransactionMax")}:{" "}
                {formatCurrency(limits.perTransactionLimitCents)}
              </p>
            </div>
          )}
        </div>
              ...
              <Tabs value={tab} onValueChange={(v) => setTab(v as "domestic" | "international")}>
                <TabsList className="mb-4">
                  <TabsTrigger value="domestic">
                    ...
                    {t("wireTransfer.domestic")}
                  </TabsTrigger>
                  <TabsTrigger value="international">
                    <Globe className="w-4 h-4 mr-2" />
                    {t("wireTransfer.international")}
                  </TabsTrigger>
                </TabsList>
                ...
                <div className="grid gap-4 max-w-lg">
                  <div>
                    ...
                    <Select value={fromAccountId} onValueChange={setFromAccountId}>
                      <SelectTrigger>
                        <SelectValue placeholder={t("wireTransfer.selectAccount")} />
                      </SelectTrigger>
                      <SelectContent>
                        {accounts.map((a) => (
                          <SelectItem key={a.id} value={a.id}>
                            {a.nickname || a.type} ({a.accountNumberMasked}) -{" "}
                            {formatCurrency(a.availableBalanceCents)}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder={t("wireTransfer.beneficiaryNamePlaceholder")}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      placeholder={t("wireTransfer.bankNamePlaceholder")}
                    />
                  </div>
                  ...
                  <TabsContent value="domestic" className="mt-0 space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={t("wireTransfer.routingNumberPlaceholder")}
                        maxLength={9}
                      />
                      {routingNumber && !/^\d{9}$/.test(routingNumber) && (
                        <p className="text-xs text-destructive mt-1">
                          {t("wireTransfer.validation.routingNumber")}
                        </p>
                      )}
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={t("wireTransfer.accountNumberPlaceholder")}
                      />
                    </div>
                  </TabsContent>
                  ...
                  <TabsContent value="international" className="mt-0 space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={t("wireTransfer.swiftCodePlaceholder")}
                        maxLength={11}
                      />
                      {swiftCode && !/^[A-Z]{6}[A-Z0-9]{2}([A-Z0-9]{3})?$/.test(swiftCode) && (
                        <p className="text-xs text-destructive mt-1">
                          {t("wireTransfer.validation.swiftCode")}
                        </p>
                      )}
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={t("wireTransfer.ibanPlaceholder")}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        placeholder={t("wireTransfer.bankCountryPlaceholder")}
                      />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <Select value={purpose} onValueChange={setPurpose}>
                      <SelectTrigger>
                        <SelectValue placeholder={t("wireTransfer.selectPurpose")} />
                      </SelectTrigger>
                      <SelectContent>
                        {(
                          [
                          ...
                          <SelectItem key={key} value={t(`wireTransfer.purposeOptions.${key}`)}>
                            {t(`wireTransfer.purposeOptions.${key}`)}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
                  ...
                  <div>
                    ...
                    <Textarea
                      ...
                      placeholder={t("wireTransfer.memoPlaceholder")}
                      rows={2}
                    />
                  </div>
              ...
              <div className="max-w-lg space-y-4">
                ...
                <div className="rounded-lg border p-4 space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t("wireTransfer.summary.type")}</span>
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t("wireTransfer.summary.to")}</span>
                    <span className="font-medium">{beneficiaryName}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t("wireTransfer.summary.bank")}</span>
                    <span className="font-medium">{bankName}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">
                      {t("wireTransfer.summary.amount")}
                    </span>
                    <span className="font-medium">{formatCurrency(amountCents)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">{t("wireTransfer.summary.fee")}</span>
                    <span className="font-medium">{formatCurrency(feeCents)}</span>
                  </div>
                  <Separator />
                  <div className="flex justify-between font-semibold">
                    <span>{t("wireTransfer.summary.total")}</span>
                    <span>{formatCurrency(amountCents + feeCents)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">
                      {t("wireTransfer.summary.purpose")}
                    </span>
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    disabled={createDomestic.isPending || createInternational.isPending}
                  >
                    {(createDomestic.isPending || createInternational.isPending) && (
        ...
        <Card>
          ...
          <CardContent>
            {wires.length === 0 ? (
              <EmptyState
                icon={ArrowUpRight}
                title={t("wireTransfer.noWireTransfers")}
                description={t("wireTransfer.noWireTransfersDescription")}
              />
            ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {wire.bankName} - {wire.referenceNumber}
                          </p>
                          <p className="text-xs text-muted-foreground flex items-center gap-1">
                            <Clock className="w-3 h-3" />
                            {new Date(wire.createdAt).toLocaleDateString(undefined)}
                          </p>
                        </div>
                      </div>
                      <div className="text-right flex items-center gap-3">
                        <div>
                          <p className="font-semibold text-sm">
                            {formatCurrency(wire.amountCents)}
                          </p>
                        ...
                        {wire.status === "pending" && (
                          <Button variant="ghost" size="icon" onClick={() => setCancelId(wire.id)}>
      ...
      <AlertDialog open={!!cancelId} onOpenChange={() => setCancelId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t("wireTransfer.cancelDialog.description")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => cancelId && handleCancel(cancelId)}>
              {t("wireTransfer.cancelDialog.cancelWire")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
