...
export default function DeviceManagement() {
  ...
  const handleRemove = async (deviceId: string) => {
    try {
      ...
      toast({
        title: t("deviceManagement.deviceRemoved"),
        description: t("deviceManagement.deviceRemovedDesc"),
      });
  ...
  const handleTrust = async (device: RegisteredDevice) => {
    try {
      if (device.isTrusted) {
        ...
        toast({
          title: t("deviceManagement.trustRemoved"),
          description: t("deviceManagement.trustRemovedDesc"),
        });
      } else {
        await trustDevice.mutateAsync(device.id);
        toast({
          title: t("deviceManagement.deviceTrusted"),
          description: t("deviceManagement.deviceTrustedDesc"),
        });
      }
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 space-y-6">
      ...
      {devices.length === 0 ? (
        <EmptyState
          icon={Monitor}
          title={t("deviceManagement.noDevices")}
          description={t("deviceManagement.noDevicesDesc")}
        />
      ) : (
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {device.isCurrent && (
                            <Badge variant="default">{t("deviceManagement.current")}</Badge>
                          )}
                          {device.isTrusted && (
                            <Badge variant="outline" className="text-green-700 border-green-300">
                              <Shield className="h-3 w-3 mr-1" />
                              {t("deviceManagement.trusted")}
                            </Badge>
                          )}
                        </div>
                    ...
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        title={
                          device.isTrusted
                            ? t("deviceManagement.removeTrust")
                            : t("deviceManagement.trustDevice")
                        }
                      >
                        {device.isTrusted ? (
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => setRenameDialog({ id: device.id, name: device.name })}
                        title={t("deviceManagement.rename")}
                      >
                      ...
                      {!device.isCurrent && (
                        <Button
                          ...
                          title={t("deviceManagement.removeDevice")}
                        >
                      ...
                      <Button
                        ...
                        title={t("deviceManagement.activityLog")}
                      >
                        {isExpanded ? (
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => confirmRemove && handleRemove(confirmRemove)}>
              {t("deviceManagement.removeDevice")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
...
function DeviceActivityLog({ deviceId }: { deviceId: string }) {
  ...
  if (isLoading) {
    return (
      <div className="mt-4 pl-10 text-sm text-muted-foreground">
        {t("deviceManagement.loadingActivity")}
      </div>
    );
  }
  ...
  if (activity.length === 0) {
    return (
      <div className="mt-4 pl-10 text-sm text-muted-foreground">
        {t("deviceManagement.noRecentActivity")}
      </div>
    );
  }
  ...
  return (
    <div className="mt-4 pl-10 border-t pt-3">
      <p className="text-xs font-medium text-muted-foreground mb-2">
        {t("deviceManagement.recentActivity")}
      </p>
