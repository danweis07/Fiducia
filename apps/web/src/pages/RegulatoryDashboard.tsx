...
import { useSafeguarding, useInterestWithholding, useCarbonSummary } from "@/hooks/useRegulatory";
...
export default function RegulatoryDashboard() {
  ...
  const { data: carbonData, isLoading: carbonLoading } = useCarbonSummary(
    "2026-01-01",
    "2026-03-31",
  );

  if (sgLoading && whLoading && carbonLoading)
    return (
      <AppShell>
        <PageSkeleton />
      </AppShell>
          ...
          <TabsContent value="safeguarding">
            <div className="space-y-4">
              ...
              {sgLoading ? (
                ...
                <EmptyState
                  icon={Building}
                  title={t("regulatory.safeguarding.noData")}
                  description={t("regulatory.safeguarding.noDataDesc")}
                />
              ) : (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm mt-2">
                              <div>
                                <span className="text-muted-foreground">
                                  {t("regulatory.safeguarding.protectionScheme")}:
                                </span>
                                <p className="font-medium">{item.protectionScheme}</p>
                              </div>
                              <div>
                                <span className="text-muted-foreground">
                                  {t("regulatory.safeguarding.protectionLimit")}:
                                </span>
                                <p className="font-bold text-green-600">{item.protectionLimit}</p>
                              </div>
                              <div>
                                <span className="text-muted-foreground">
                                  {t("regulatory.safeguarding.regulator")}:
                                </span>
                                <p className="font-medium">{item.regulatoryBody}</p>
                              </div>
                              <div>
                                <span className="text-muted-foreground">
                                  {t("regulatory.safeguarding.lastAudit")}:
                                </span>
                            ...
                            {item.certificateUrl && (
                              ...
                              >
                                {t("regulatory.safeguarding.viewRegistration")}{" "}
          ...
          <TabsContent value="withholding">
            <div className="space-y-4">
              ...
              {withholdingData && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Card>
                    <CardContent className="pt-4 text-center">
                      <p className="text-sm text-muted-foreground">
                        {t("regulatory.withholding.grossInterest")}
                      </p>
                  ...
                  <Card className="border-red-200">
                    <CardContent className="pt-4 text-center">
                      <p className="text-sm text-muted-foreground">
                        {t("regulatory.withholding.taxWithheld")}
                      </p>
                  ...
                  <Card className="border-green-200">
                    <CardContent className="pt-4 text-center">
                      <p className="text-sm text-muted-foreground">
                        {t("regulatory.withholding.netInterest")}
                      </p>
              ...
              {whLoading ? (
                ...
                <EmptyState
                  icon={Receipt}
                  title={t("regulatory.withholding.noEntries")}
                  description={t("regulatory.withholding.noEntriesDesc")}
                />
              ) : (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {t("regulatory.withholding.ledgerDescription")}
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                        ...
                        <thead>
                          <tr className="border-b">
                            <th className="text-left py-2 font-medium text-muted-foreground">
                              {t("regulatory.withholding.period")}
                            </th>
                            <th className="text-left py-2 font-medium text-muted-foreground">
                              {t("regulatory.withholding.jurisdiction")}
                            </th>
                            <th className="text-right py-2 font-medium text-muted-foreground">
                              {t("regulatory.withholding.gross")}
                            </th>
                            <th className="text-right py-2 font-medium text-muted-foreground">
                              {t("regulatory.withholding.taxWithheldColumn")}
                            </th>
                            <th className="text-right py-2 font-medium text-muted-foreground">
                              {t("regulatory.withholding.net")}
                            </th>
                            <th className="text-right py-2 font-medium text-muted-foreground">
                              {t("regulatory.withholding.rate")}
                            </th>
                          </tr>
                        </thead>
                        <tbody>
                          {withholdingEntries.map((entry: InterestWithholdingEntry) => (
                            <tr key={entry.entryId} className="border-b last:border-0">
                              ...
                              <td className="py-2 text-right font-medium">
                                {formatCurrencyIntl(entry.grossInterestCents, entry.currency)}
                              </td>
                              <td className="py-2 text-right text-red-600">
                                {formatCurrencyIntl(entry.taxWithheldCents, entry.currency)}
                              </td>
                              <td className="py-2 text-right font-medium text-green-600">
                                {formatCurrencyIntl(entry.netInterestCents, entry.currency)}
                              </td>
                              <td className="py-2 text-right">
                                {(entry.withholdingRateBps / 100).toFixed(1)}%
                              </td>
                            </tr>
                          ))}
                        </tbody>
          ...
          <TabsContent value="carbon">
            <div className="space-y-4">
              {carbonLoading ? (
                ...
                <EmptyState
                  icon={Leaf}
                  title={t("regulatory.carbon.noData")}
                  description={t("regulatory.carbon.noDataDesc")}
                />
              ) : (
                <>
                  {/* Carbon Score Card */}
                    ...
                    <CardContent className="pt-6">
                      <div className="text-center">
                        <div className="flex items-center justify-center gap-2 mb-2">
                          ...
                          <p className="text-sm text-muted-foreground">
                            {t("regulatory.carbon.footprintLabel")}
                          </p>
                        </div>
                        <p className="text-4xl font-bold">
                          {t("regulatory.carbon.kgCO2", { value: carbon.totalCarbonKg.toFixed(1) })}
                        </p>
                          ...
                          <span className="text-sm text-muted-foreground">
                            {carbon.monthOverMonthChange < 0 ? (
                              <span className="text-green-600 flex items-center gap-0.5">
                                ...
                                {t("regulatory.carbon.vsLastPeriod", {
                                  percent: Math.abs(carbon.monthOverMonthChange),
                                })}
                              </span>
                            ) : (
                              <span className="text-red-600">
                                {t("regulatory.carbon.vsLastPeriodUp", {
                                  percent: carbon.monthOverMonthChange,
                                })}
                              </span>
                            )}
                          </span>
                        ...
                        <p className="text-sm text-muted-foreground mt-2">
                          {t("regulatory.carbon.countryAverage", {
                            avg: carbon.countryAvgKg.toFixed(0),
                            percent: ((carbon.totalCarbonKg / carbon.countryAvgKg) * 100).toFixed(
                              0,
                            ),
                          })}
                        </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Card>
                      <CardContent className="pt-4 text-center">
                        <p className="text-sm text-muted-foreground">
                          {t("regulatory.carbon.transactionsTracked")}
                        </p>
                    ...
                    <Card>
                      <CardContent className="pt-4 text-center">
                        <p className="text-sm text-muted-foreground">
                          {t("regulatory.carbon.avgPerTransaction")}
                        </p>
                        <p className="text-2xl font-bold mt-1">
                          {t("regulatory.carbon.kgValue", {
                            value: carbon.avgCarbonPerTransaction.toFixed(1),
                          })}
                        </p>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardContent className="pt-4 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <p className="text-sm text-muted-foreground">
                            {t("regulatory.carbon.offsetCost")}
                          </p>
                        </div>
                        <p className="text-2xl font-bold mt-1">
                          {formatCurrencyIntl(carbon.offsetCostCents, carbon.offsetCurrency)}
                        </p>
                      </CardContent>
                    </Card>
                  </div>
                        ...
                        {carbon.topCategories.map((cat) => (
                          <div key={cat.category} className="space-y-1">
                            <div className="flex items-center justify-between text-sm">
                              ...
                              <span>
                                {cat.carbonKg.toFixed(1)} kg ({cat.percentage.toFixed(1)}%)
                              </span>
                            </div>
