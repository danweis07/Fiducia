  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>{t("loanApplication.chooseTypeDesc")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {products.length === 0 && (
          <p className="text-muted-foreground text-sm">{t("loanApplication.noProducts")}</p>
        )}
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="secondary">
                    {t(LOAN_TYPE_LABEL_KEYS[product.loanType] || product.loanType)}
                  </Badge>
                </div>
                {product.description && (
                ...
                <div className="flex gap-4 mt-2 text-sm text-muted-foreground">
                  <span>
                    {t("loanApplication.rate")}: {(product.interestRateBps / 100).toFixed(2)}%{" "}
                    {product.rateType}
                  </span>
                  <span>
                  ...
                  <span>
                    {product.minTermMonths}&ndash;{product.maxTermMonths}{" "}
                    {t("loanApplication.months")}
                  </span>
                </div>
              </div>
            </button>
