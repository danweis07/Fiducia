  ...
  const handleFrontCapture = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        toast({
          title: t("deposit.captureError"),
          description: t("deposit.captureErrorDesc"),
          variant: "destructive",
        });
      }
    }
        ...
        <Card>
          <CardContent className="py-4">
            <SuccessAnimation
              ...
              description={t("deposit.depositSubmittedDesc", {
                amount: formatCurrency(amountCents),
                account: `${selectedAccount?.nickname} (${selectedAccount?.accountNumberMasked})`,
              })}
              details={[
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>{t("deposit.frontOfCheckDesc")}</CardDescription>
          </CardHeader>
          <CardContent>
            <div
            ...
            >
              {frontImage ? (
                <div className="text-center space-y-1">
                  ...
                  <p className={`text-sm ${captureColors.captured.text}`}>
                    {t("deposit.frontCaptured")}
                  </p>
                </div>
              ) : (
                <div className="text-center space-y-2">
                  <Camera className="h-8 w-8 text-muted-foreground mx-auto" />
                  <p className="text-sm text-muted-foreground">{t("deposit.tapToCapture")}</p>
                </div>
              )}
            </div>
            <Button
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>{t("deposit.backOfCheckDesc")}</CardDescription>
          </CardHeader>
          <CardContent>
            <div
              className={`border-2 border-dashed rounded-lg flex flex-col items-center justify-center h-40 transition-colors ${
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            />
            {amountCents > 0 && (
              <p className="text-sm text-muted-foreground">{formatCurrency(amountCents)}</p>
            )}
          </div>
