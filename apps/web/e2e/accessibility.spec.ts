...
test.describe("Accessibility", () => {
  test.describe("Heading Hierarchy", () => {
    for (const pageInfo of PAGES) {
      test(`${pageInfo.name}: should have proper heading hierarchy (h1 > h2 > h3)`, async ({
        page,
      }) => {
  ...
  test.describe("Keyboard Navigation", () => {
    ...
    test("links should be focusable and show focus indicators", async ({ page }) => {
      ...
      if (count > 0) {
        ...
        const hasFocusStyle = await firstLink.evaluate((el) => {
          ...
          return (
            (outline !== "none" && outline !== "") || (boxShadow !== "none" && boxShadow !== "")
          );
        });
    ...
    test("sort dropdown should be keyboard accessible", async ({ page }) => {
      ...
      if (isVisible) {
        ...
        const isFocused = await sortSelect.evaluate((el) => document.activeElement === el);
        expect(isFocused).toBeTruthy();
  ...
  test.describe("Color Contrast (WCAG AA)", () => {
    test("main heading text should have sufficient contrast against background", async ({
      page,
    }) => {
    ...
    test("transition score badges should have contrasting text against colored background", async ({
      page,
    }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
  ...
  test.describe("Screen Reader Announcements for Dynamic Content", () => {
    ...
    test("live regions or status updates should exist for dynamic content changes", async ({
      page,
    }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
    ...
    test("loading states should be communicated to assistive technology", async ({ page }) => {
      ...
      const _hasLoading = await loadingIndicators
        .first()
  ...
  test.describe("Semantic HTML", () => {
    ...
    test("interactive elements should not use div or span as clickable without role", async ({
      page,
    }) => {
      await page.goto("/");
      await page.waitForLoadState("networkidle");
